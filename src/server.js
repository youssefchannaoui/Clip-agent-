...
async function route(req, res, url) {
  ...
  // Stream the original lecture into the editor. Using the rendered clip as
  // a fallback burns the existing captions into the preview and then the
  // editor draws a second live caption layer over it.
  ...
  if (method === 'GET' && clipSourcePreview) {
    ...
    if (!project?.sourceFile || !fs.existsSync(project.sourceFile)) {
      return json(res, 404, { error: 'Original lecture source is unavailable.' });
    }
