...
function appState(user = null) {
  ...
  return {
    ...
    publishingSettings: publishingSettings(), social: social.connectionStatus(), billing: billing.publicBilling(user),
  };
}
...
async function route(req, res, url) {
  ...
  if (method === 'POST' && pathname === '/api/videos') {
    ...
    for (const source of urls) {
      try { results.push({ url: source, ok: true, projectId: await agent.submitVideo(source, body.title || '', currentUser?.id || auth.defaultOwnerId(), { sourceRange, sourceMeta }) }); }
