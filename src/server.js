...
async function route(req, res, url) {
  ...
  // The editor must preview the untouched lecture source. Falling back to the
  // already-rendered clip burns captions/watermarks into the video and then the
  // editor overlays them a second time, which looks like two videos and two
  // caption layers. Stream the project's original source instead.
  const clipSourcePreview = pathname.match(/^\/api\/clips\/([^/]+)\/source-preview$/);
  if (method === 'GET' && clipSourcePreview) {
    const id = decodeURIComponent(clipSourcePreview[1]);
    ...
    if (!project?.sourceFile || !fs.existsSync(project.sourceFile)) {
      return json(res, 404, { error: 'Original source video is unavailable.' });
    }
