...
function queueTemplateRefreshForUnposted(template) {
  if (!template?.id) return { queued: 0, skipped: 0, errors: [] };
  let queued = 0; let skipped = 0; const errors = [];
  for (const clip of state.clips) {
    if (clip.variantOf || clip.status === 'posted') { skipped += 1; continue; }
    const active = state.rerenderJobs.find(job => job.clipId === clip.id && ['queued', 'processing'].includes(job.status));
    if (active && active.templateId === template.id) { skipped += 1; continue; }
    try {
  ...
  if (queued || errors.length) log(`Template "${template.name}" queued ${queued} unposted clip${queued === 1 ? '' : 's'} for refresh; ${skipped} skipped.`);
...
async function route(req, res, url) {
  ...
  if (method === 'POST' && pathname === '/api/templates') {
    ...
    try {
      ...
      let refresh = { queued: 0, skipped: 0, errors: [] };
      if (body.select !== false) {
        templates.setSelectedTemplate(template.id);
        refresh = queueTemplateRefreshForUnposted(template);
      }
      log(`Created template "${template.name}". It is ready for automated renders.`);
      return json(res, 200, { ok: true, template, refresh });
    } catch (error) { return json(res, 400, { error: error.message }); }
  }
  ...
  if (method === 'PUT' && templateMatch) {
    ...
    try {
      ...
      const refresh = selected?.id === template.id ? queueTemplateRefreshForUnposted(template) : { queued: 0, skipped: 0, errors: [] };
  ...
  if (method === 'POST' && pathname === '/api/template') {
    ...
    try {
      ...
      const refresh = queueTemplateRefreshForUnposted(template);
