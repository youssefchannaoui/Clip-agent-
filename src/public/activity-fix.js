...
function renderHome(){
  ...
  panel.innerHTML = `<section class="dc-v3-hero" data-tour="home-hero"><div><div class="dc-v3-kicker">${ICON.scissors} V3 workspace</div><h1 class="dc-v3-title">Create clean dawah clips without staring at a wall of text.</h1><p class="dc-v3-copy">Paste a lecture, pick the template, review the thumbnails, then publish. The dashboard now shows the workflow visually so new users know what to do next.</p><div class="dc-v3-actions"><button class="dc-btn" id="dcHeroCreate">Start with YouTube</button><button class="dc-btn secondary" id="dcHeroTour">Run demo tour</button><button class="dc-btn secondary" data-dc-nav="templates">Open templates</button></div><div class="dc-v3-pipeline">${workflowStep('youtube','Import','Paste YouTube or a video URL')}${workflowStep('captions','Clip','AI captions and selects moments')}${workflowStep('publish','Post','Send to connected platforms')}</div></div><div class="dc-v3-phone-wall" aria-label="Recent clip thumbnails">${heroThumbs(clips)}</div></section><div class="dc-v3-source-row" data-tour="source-options"><button class="dc-v3-source" type="button" id="dcSourceYouTube"><span class="dc-v3-platform youtube">${ICON.play}</span><strong>YouTube import</strong><span>Paste a lecture and begin processing.</span></button><button class="dc-v3-source" type="button" data-dc-nav="templates"><span class="dc-v3-platform template">${ICON.style}</span><strong>${esc(selectedTemplate)}</strong><span>Current default look for every render.</span></button><button class="dc-v3-source" type="button" data-dc-nav="publishing"><span class="dc-v3-platform publish">${ICON.social}</span><strong>Publishing setup</strong><span>Connect TikTok, Instagram, Facebook and YouTube.</span></button></div><section class="dc-card dc-create-card v3" data-tour="create-form"><div class="dc-create-grid"><input id="dcCreateUrl" placeholder="Paste YouTube or video URL"><select id="dcCreateTemplate" data-tour="template-picker">${(d.templates||[]).map(t=>`<option value="${esc(t.id)}" ${t.id===d.selectedTemplate?.id?'selected':''}>${esc(t.name)}</option>`).join('')}</select><select id="dcCreateCount"><option>4</option><option selected>8</option><option>12</option><option>16</option></select><select id="dcCreateDuration"><option value="15,45">15–45 sec</option><option value="30,60" selected>30–60 sec</option><option value="45,90">45–90 sec</option></select><button class="dc-btn" id="dcGenerate" data-tour="generate-button">Generate clips</button></div></section><div class="dc-home-metrics v3">${metric(projects.length,'Lectures')}${metric(clips.length,'Generated clips')}${metric(waiting,'Waiting review')}${metric(scheduled,'Scheduled')}${metric(posted,'Published')}</div><div class="dc-home-grid"><div class="dc-stack"><section class="dc-card dc-card-pad v3-card" data-tour="happening-now"><div class="dc-card-head"><div><h2>Happening now</h2><p>Generation, re-renders and uploads update live.</p></div><span class="dc-pill ${jobs.length?'warn':'good'}">${jobs.length?`${jobs.length} active`:'All clear'}</span></div><div class="dc-now-list">${jobs.length?jobs.map(jobRow).join(''):latestActivity(d)}</div></section><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Recent projects</h2><p>Continue from the last lecture you worked on.</p></div><button class="dc-btn secondary" data-dc-nav="projects">View all</button></div><div class="dc-row-list">${recentProjects(projects,clips)}</div></section></div><div class="dc-stack dc-command-card"><section class="dc-card dc-card-pad v3-card" data-tour="platform-cards"><div class="dc-card-head"><div><h2>Platform connections</h2><p>Status before scheduling.</p></div><button class="dc-btn secondary" data-dc-nav="publishing">Manage</button></div><div class="dc-social-grid">${socialCards(d)}</div></section><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Upcoming posts</h2><p>${scheduled} clip${scheduled===1?'':'s'} approved or scheduled.</p></div><button class="dc-btn secondary" data-dc-nav="schedule">Open publish</button></div><div class="dc-row-list">${upcomingRows(clips)}</div></section><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Needs attention</h2><p>Only workflow blockers appear here.</p></div></div><div class="dc-row-list">${attentionRows(d)}</div></section></div></div>`;
