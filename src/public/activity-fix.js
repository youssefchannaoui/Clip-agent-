...
const STUDIO_NAV = [
  ['templates','Clip Styles','style'], ['brand','Brand Kit','brand'], ['lab','Creator Lab','lab'],
...
function labDimensionRows(clip){const d=clip.scoreBreakdown||clip.quality?.scoreBreakdown||{};return[['Hook','hook'],['Flow','flow'],['Value','value'],['Clarity','clarity'],['Complete','completeness'],['Specific','specificity'],['Timing','confidence']].map(([label,key])=>{const value=clamp(Number(d[key]??clip[key]??clip.quality?.[key]??0),0,100);return `<span><b>${esc(label)}</b><i><em style="width:${value}%"></em></i><strong>${Math.round(value)}</strong></span>`}).join('')}
function renderCreatorLab(){
  ...
  const intelligenceClip=scored.find(c=>c.scoreBreakdown||c.quality?.scoreBreakdown)||scored[0];const growth=intelligenceClip?.growthPack||{};const intelligencePanel=intelligenceClip?`<section class="dc-lab-panel dc-lab-intelligence standalone"><div class="dc-lab-head"><div><span>Explainable intelligence</span><h2>Why this clip can work</h2></div><button class="dc-btn secondary" data-edit-video-clip="${esc(intelligenceClip.id)}">Open in Editor</button></div><div class="dc-lab-intelligence-grid"><div><strong>${esc(shortText(intelligenceClip.title||'Top clip',76))}</strong><p>${esc((intelligenceClip.scoreReasons||[]).join(' · ')||'Generate a new clip to see its detailed retention signals.')}</p><div class="dc-lab-dimensions">${labDimensionRows(intelligenceClip)}</div></div><aside><small>Growth pack</small><h3>${esc(growth.primaryTitle||intelligenceClip.title||'Suggested title')}</h3><p>${esc((growth.alternateTitles||[]).join(' · ')||'Alternate titles appear on newly generated clips.')}</p><div>${(growth.searchTerms||[]).slice(0,6).map(term=>`<span>${esc(term)}</span>`).join('')}</div><button class="dc-btn" id="dcCopyGrowthPack">Copy post pack</button></aside></div></section>`:'';
