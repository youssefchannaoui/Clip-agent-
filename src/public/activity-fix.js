...
const CUSTOM = new Set(['home','projects','review','editor','publishing','music','automation']);
...
function injectShell(){
  ...
  const main = $('.main-col');
  if (main) {
    for (const name of ['home','projects','review','editor','publishing','music','automation']) {
      if (!$(`#view-${name}`)) {
...
function renderConnections(){
  ...
  panel.innerHTML=`<div class="dc-manage-page"><section class="dc-manage-hero"><div><span class="dc-manage-kicker">${ICON.social} Publishing control</span><h1>Connect once, publish everywhere.</h1><p>Keep destination status, TikTok checks and automatic posting rules in one clean page.</p></div><div class="dc-manage-metrics"><span><b>${connected}</b><em>connected</em></span><span><b>${enabled}</b><em>enabled</em></span><span><b>${d.directPublishingEnabled?'On':'Off'}</b><em>server flag</em></span></div></section><div class="dc-manage-grid">${providers.map(connectionCard).join('')}</div><section class="dc-settings-panel"><h2>Automatic posting destinations</h2><p>Choose which connected platforms DeenClipped can use when clips are approved or scheduled.</p><div class="dc-settings-form">${providers.map(p=>`<label class="dc-switch-row"><span><strong>${esc(providerTitle(p.provider))}</strong><span>${esc(p.account?.name||'No account selected')}</span></span><input type="checkbox" id="dcPub_${esc(p.provider)}" ${p.enabled?'checked':''} ${!p.connected?'disabled':''}></label>`).join('')}<button class="dc-btn wide" id="dcSavePublishing">Save publishing rules</button></div></section></div>`;
...
function renderCurrent(){if(currentView==='home')renderHome();if(currentView==='projects')renderProjects();if(currentView==='review')renderReview();if(currentView==='editor')ensureEditor();if(currentView==='publishing')renderConnections();if(currentView==='music')renderAudioLibrary();if(currentView==='automation')renderSettingsPage()}
