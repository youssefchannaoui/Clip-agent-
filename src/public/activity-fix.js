...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    ...
    editor.draft={...clone(template),...(saved?.draft||{}),__clipId:clip.id};editor.draft.cropPositionX??=50;editor.draft.cropPositionY??=50;editor.draft.captionTimingOffsetMs??=-120;editor.draft.captionPositionX??=captionPresetPoint(editor.draft.captionPosition,editor.draft.captionHorizontal).x;editor.draft.captionPositionY??=captionPresetPoint(editor.draft.captionPosition,editor.draft.captionHorizontal).y;
...
function bindCaptionDrag(){
  ...
  overlay.onpointerdown=event=>{
    ...
    drag={pointerId:event.pointerId,startClientX:event.clientX,startClientY:event.clientY,startX:Number(editor.draft.captionPositionX??50),startY:Number(editor.draft.captionPositionY??58),rect};
    overlay.classList.add('is-dragging','is-selected');overlay.setPointerCapture?.(event.pointerId);event.stopPropagation();event.preventDefault();
  };
  ...
  const finish=event=>{
    ...
    drag=null;hideGuides();overlay.classList.remove('is-dragging');markEditorDirty();pushHistory();
  };
