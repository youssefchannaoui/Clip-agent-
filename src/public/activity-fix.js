...
const editor = {
  ...
  framingMessage:'Smart framing has not been analysed', framingRequest:0, framingTimer:null,
};
...
function framingStatusCopy(){
  if(editor.framingStatus==='analysing')return['Analysing speaker','Sampling the source video and following the most persistent face.'];
  if(editor.framingStatus==='ready'){
    const plan=editor.framingPlan||{},confidence=Math.round(Number(plan.confidence||0)*100),method=String(plan.method||'speaker').replace(/-/g,' ');
    return['Smart framing ready',`${method}${confidence?` · ${confidence}% confidence`:''}${plan.keyframes?.length?` · ${plan.keyframes.length} tracking points`:''}`];
  }
  if(editor.framingStatus==='failed')return['Smart framing unavailable',editor.framingMessage||'Use Left, Centre or Right speaker focus.'];
  return['Smart framing','Switch to Fill / crop and enable Smart framing to analyse the speaker.'];
}
function requestFramingPlan(immediate=false){
  ...
  editor.framingTimer=setTimeout(async()=>{
    const request=++editor.framingRequest;editor.framingStatus='analysing';editor.framingMessage='Sampling the lecture frames';updateEditorPreview();if(editor.tool==='canvas')renderEditorTool();
    try{
      ...
      editor.framingPlan=result.plan;editor.framingStatus='ready';editor.framingMessage='Speaker tracking is ready';markEditorDirty(false);
    }catch(error){
      if(request!==editor.framingRequest)return;
...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    ...
    editor.draft={...clone(template),...(saved?.draft||{}),__clipId:clip.id};
    ...
    editor.captionWords=approximateWords(editor.captionText,editor.trimOut);editor.captionSource='fallback';
...
async function loadCaptionWords(clip){
  try{
    ...
    if(Array.isArray(payload.words)&&payload.words.length){
      ...
      editor.captionSource=payload.exact?'whisper':'edited';if(!editor.dirty&&payload.transcript)editor.captionText=payload.transcript;
    }
...
function renderEditor(clip){
  ...
  bindVideo(clip);renderEditorTool();updateEditorPreview();renderTimeline();requestAnimationFrame(()=>animatePanel(panel));
...
function captionTool(){
  return `<div class="dc-subtabs">${['styles','text','format','position'].map(x=>`<button class="${editor.captionTab===x?'on':''}" data-caption-tab="${x}">${x[0].toUpperCase()+x.slice(1)}</button>`).join('')}</div>${editor.captionTab==='styles'?captionStyles():editor.captionTab==='text'?captionTextPanel():editor.captionTab==='format'?captionFormat():captionPosition()}`;
}
...
function captionPosition(){
  ...
  return `<div class="dc-section"><h3>Position</h3><div class="dc-position-grid">${positions.map(([p,l])=>`<button class="${p===key?'on':''}" data-caption-position="${p}">${l}</button>`).join('')}</div></div><div class="dc-section"><h3>Margins</h3>${rangeField('Vertical margin','captionMarginV',20,800,5)}${rangeField('Horizontal margin','captionMarginH',20,700,5)}</div>`;
}
function canvasTool(){
  const [stateTitle,stateText]=framingStatusCopy(),fill=editor.draft.fitMode||'contain';
  return `<div class="dc-section"><h3>Fill mode</h3><div class="dc-fill-options"><button class="dc-fill-option ${fill==='contain'?'on':''}" data-fill-mode="contain"><span class="dc-fill-swatch contain"></span><b>Fit</b><span>Show full frame</span></button><button class="dc-fill-option ${fill==='blur'?'on':''}" data-fill-mode="blur"><span class="dc-fill-swatch blur"></span><b>Blur</b><span>Fill empty space</span></button><button class="dc-fill-option ${fill==='crop'?'on':''}" data-fill-mode="crop"><span class="dc-fill-swatch crop"></span><b>Fill</b><span>Crop to canvas</span></button></div></div><div class="dc-section"><h3>Canvas</h3>${selectField('Aspect ratio','__ratio',[['9:16','Vertical 9:16'],['4:5','Portrait 4:5'],['1:1','Square 1:1'],['16:9','Landscape 16:9']])}${editor.draft.fitMode==='blur'?rangeField('Blur strength','blurStrength',0,60,1):''}${editor.draft.fitMode==='crop'?`${checkField('Smart framing','smartFramingEnabled')}${selectField('Speaker focus','smartFramingBias',[['auto','Automatic'],['left','Left'],['center','Centre'],['right','Right']])}${rangeField('Framing zoom','smartFramingZoom',.75,1.35,.05)}${rangeField('Subject padding','smartFramingPadding',.05,.45,.01)}${rangeField('Camera smoothing','smartFramingSmoothing',0,.95,.05)}<div class="dc-framing-state ${editor.framingStatus}"><i></i><div><strong>${esc(stateTitle)}</strong><span>${esc(stateText)}</span></div></div><button class="dc-btn secondary" id="dcAnalyseFraming" style="width:100%;margin-top:7px" ${editor.framingStatus==='analysing'?'disabled':''}>${editor.framingStatus==='analysing'?'Analysing…':'Analyse framing again'}</button>`:''}</div><div class="dc-section"><h3>Quick layouts</h3><div class="dc-style-grid"><button class="dc-style-card" data-quick-layout="speaker"><b>Speaker focus</b><span>Fill and track speaker</span></button><button class="dc-style-card" data-quick-layout="full"><b>Full frame</b><span>Show complete source</span></button><button class="dc-style-card" data-quick-layout="cinematic"><b>Cinematic</b><span>Film look and low captions</span></button><button class="dc-style-card" data-quick-layout="minimal"><b>Minimal</b><span>Clean layout and branding</span></button></div></div>`;
}
function styleTool(){
...
function changeTemplateInput(input){
  ...
  if(['__ratio','smartFramingEnabled','smartFramingBias','smartFramingZoom','smartFramingPadding','smartFramingSmoothing'].includes(key))requestFramingPlan();
}
...
function applyQuickLayout(id){
  if(id==='speaker')Object.assign(editor.draft,{fitMode:'crop',smartFramingEnabled:true,smartFramingBias:'auto',smartFramingZoom:1,smartFramingPadding:.18,smartFramingSmoothing:.82,filterPreset:'natural'});
  if(id==='full')Object.assign(editor.draft,{fitMode:'contain',smartFramingEnabled:false,filterPreset:'natural'});
  if(id==='cinematic')Object.assign(editor.draft,{fitMode:'crop',smartFramingEnabled:true,smartFramingBias:'auto',smartFramingZoom:1.06,smartFramingPadding:.2,smartFramingSmoothing:.88,filterPreset:'cinematic',vignette:.35,captionPosition:'bottom',captionFont:'Playfair Display'});
  if(id==='minimal')Object.assign(editor.draft,{fitMode:'contain',smartFramingEnabled:false,filterPreset:'natural',captionMode:'phrase',captionHorizontal:'center',watermark:'',brandLineEnabled:false});
  markEditorDirty();pushHistory();renderEditorTool();updateEditorPreview();if(editor.draft.fitMode==='crop'&&editor.draft.smartFramingEnabled)requestFramingPlan(true);
}
...
function updateEditorPreview(){
  ...
  canvas.style.aspectRatio=`${d.width||1080}/${d.height||1920}`;canvas.dataset.fill=d.fitMode||'contain';canvas.dataset.framing=d.fitMode==='crop'&&d.smartFramingEnabled?editor.framingStatus:'off';canvas.style.setProperty('--dc-canvas-background',d.frameBackground||'#000000');
  if(bg){bg.style.filter=`blur(${Math.max(0,Number(d.blurStrength||28))}px) brightness(.72)`;bg.style.display=d.fitMode==='blur'?'block':'block'}
