  ...
  panel.innerHTML=`<div class="dc-brand-page">
    ...
    <div class="dc-brand-layout"><section class="dc-brand-preview-card"><div class="dc-brand-phone"><img src="/marketing-assets/reel-beneficial.webp" alt="Brand watermark preview"><span class="dc-brand-watermark ${enabled?'':'off'}" id="dcBrandPreviewMark" style="--brand-color:${esc(brand.watermarkColor||'#D9B478')};--brand-opacity:${Number(brand.watermarkOpacity||88)/100}" data-position="${esc(brand.watermarkPosition||'top-center')}">${esc(text)}</span><i id="dcBrandPreviewLine" class="${brand.brandLineEnabled&&premium?'on':''}" style="--brand-color:${esc(brand.brandLineColor||'#D9B478')}"></i><div class="dc-brand-caption">WHAT YOU DO<br><em>consistently</em></div></div><div class="dc-brand-preview-copy"><strong>Live export preview</strong><span>Position, colour and visibility update as you edit.</span></div></section>
      <section class="dc-brand-controls"><div class="dc-brand-entitlement ${premium?'premium':'free'}"><span>${premium?ICON.check:ICON.warning}</span><div><strong>${premium?'Full brand control':'Free exports stay branded'}</strong><p>${premium?'Turn the watermark off for clean exports, use your own name, and add a branded colour line.':'Every free-token render includes “DEENCLIPPED”. Upgrade to switch it off or replace it with your own brand.'}</p></div>${premium?'':`<button class="dc-btn" id="dcBrandUpgrade">Unlock branding</button>`}</div>
        <form id="dcBrandForm" class="dc-brand-form"><label class="dc-switch-row wide"><span><strong>Show watermark</strong><span>${required?'Required on the free plan':'Turn branding on or off for every new render'}</span></span><input type="checkbox" name="watermarkEnabled" ${enabled?'checked':''} ${required?'disabled':''}></label><label class="wide">Watermark text<input name="watermarkText" maxlength="60" value="${esc(text)}" ${premium?'':'disabled'}></label><label class="${premium?'':'is-locked'}">Position<select name="watermarkPosition" ${premium?'':'disabled'}>${[['top-left','Top left'],['top-center','Top centre'],['top-right','Top right'],['bottom-left','Bottom left'],['bottom-center','Bottom centre'],['bottom-right','Bottom right']].map(([v,l])=>`<option value="${v}" ${(premium?brand.watermarkPosition:'top-center')===v?'selected':''}>${l}</option>`).join('')}</select></label><label class="${premium?'':'is-locked'}">Colour<input name="watermarkColor" type="color" value="${esc(premium?(brand.watermarkColor||'#D9B478'):'#D9B478')}" ${premium?'':'disabled'}></label><label class="wide ${premium?'':'is-locked'}">Opacity <b id="dcBrandOpacityValue">${premium?Number(brand.watermarkOpacity||88):88}%</b><input name="watermarkOpacity" type="range" min="20" max="100" step="1" value="${premium?Number(brand.watermarkOpacity||88):88}" ${premium?'':'disabled'}></label><label class="dc-switch-row wide ${premium?'':'is-locked'}"><span><strong>Brand accent line</strong><span>Add a subtle colour edge to every clip</span></span><input type="checkbox" name="brandLineEnabled" ${brand.brandLineEnabled&&premium?'checked':''} ${premium?'':'disabled'}></label><label class="wide ${premium?'':'is-locked'}">Accent colour<input name="brandLineColor" type="color" value="${esc(brand.brandLineColor||'#D9B478')}" ${premium?'':'disabled'}></label><div class="dc-brand-ai-head wide ${premium?'':'is-locked'}"><span>${ICON.sparkles}</span><div><strong>AI language & growth profile</strong><p>Teach DeenClipped names and terms, then tune titles and descriptions to your audience—without changing quoted wording.</p></div>${premium?'<b>Premium</b>':'<b>Locked</b>'}</div><label class="wide ${premium?'':'is-locked'}">Names and specialist vocabulary<textarea name="brandVocabulary" rows="3" maxlength="1200" placeholder="Speaker names, Arabic terms, series names…" ${premium?'':'disabled'}>${esc((brand.brandVocabulary||[]).join(', '))}</textarea></label><label class="${premium?'':'is-locked'}">Audience<select name="audience" ${premium?'':'disabled'}>${[['general','General audience'],['new-muslims','New Muslims'],['students','Students'],['families','Families'],['creators','Creators']].map(([v,l])=>`<option value="${v}" ${String(brand.audience||'general')===v?'selected':''}>${l}</option>`).join('')}</select></label><label class="${premium?'':'is-locked'}">Primary goal<select name="contentGoal" ${premium?'':'disabled'}>${[['education','Education'],['growth','Growth'],['community','Community'],['reflection','Reflection']].map(([v,l])=>`<option value="${v}" ${String(brand.contentGoal||'education')===v?'selected':''}>${l}</option>`).join('')}</select></label><label class="${premium?'':'is-locked'}">Tone<select name="brandTone" ${premium?'':'disabled'}>${[['respectful','Respectful'],['warm','Warm'],['direct','Direct'],['reflective','Reflective']].map(([v,l])=>`<option value="${v}" ${String(brand.brandTone||'respectful')===v?'selected':''}>${l}</option>`).join('')}</select></label><label class="${premium?'':'is-locked'}">Avoid phrases<input name="avoidPhrases" maxlength="600" placeholder="Clickbait phrases to avoid" value="${esc((brand.avoidPhrases||[]).join(', '))}" ${premium?'':'disabled'}></label><button class="dc-btn wide" type="submit">Save Brand Kit</button></form></section></div>
...
function qualityCheck(label,ready,copy){return `<span class="dc-v6-check ${ready?'ready':'warn'}"><i>${ready?ICON.check:ICON.warning}</i><b>${esc(label)}</b><em>${esc(copy)}</em></span>`}
...
function renderTemplatesPage(){
  ...
  panel.innerHTML=`<div class="dc-styles-page"><section class="dc-style-hero"><div><span class="dc-style-kicker">${ICON.style} Clip styles</span><h1>Choose how every new clip looks.</h1><p>A style controls captions, framing, colour and watermark placement. Preview it first, then make it the default for clips you create next.</p></div><div class="dc-style-steps"><span><b>1</b><em>Preview</em></span><i></i><span><b>2</b><em>Use for new clips</em></span><i></i><span><b>3</b><em>Fine-tune in Editor</em></span></div></section>
    <section class="dc-style-current"><span class="dc-style-current-icon">${ICON.check}</span><div><small>Current default for new clips</small><h2>${esc(selected.name||'No style selected')}</h2><p>New clips use this look automatically. Existing clips stay unchanged unless you choose “Apply to existing clips”.</p></div><div class="dc-style-current-actions"><button class="dc-btn secondary" data-preview-template="${esc(selected.id||'')}">Large preview</button>${currentAction}<button class="dc-btn" data-open-style-editor>${hasClips?'Create a custom style':'Create your first clip'}</button></div></section>
    <section class="dc-style-library"><div class="dc-style-section-head"><div><span>Ready-made</span><h2>Recommended styles</h2><p>Safe starting points designed for short vertical clips. Preview uses sample words over one of your source thumbnails.</p></div><span class="dc-style-count">${recommended.length} styles</span></div><div class="dc-style-grid">${recommendedCards||`<div class="dc-empty"><strong>No built-in styles</strong><span>Default styles could not be loaded.</span></div>`}</div></section>
    <section class="dc-style-library"><div class="dc-style-section-head"><div><span>Your studio</span><h2>Your saved styles</h2><p>Custom looks created from the clip editor. They are private to your account.</p></div><button class="dc-btn secondary" data-open-style-editor>${hasClips?'Create in Editor':'Create a clip first'}</button></div>${customCards?`<div class="dc-style-grid">${customCards}</div>`:`<div class="dc-style-empty"><span>${ICON.style}</span><div><strong>No custom styles yet</strong><p>Open a real clip, adjust captions and colour, then save the look as your default.</p></div><button class="dc-btn" data-open-style-editor>${hasClips?'Open Editor':'Create a clip'}</button></div>`}</section>
    <p class="dc-style-footnote">Style previews use sample caption text. They never alter a clip until you choose an action.</p>
  </div>`;
