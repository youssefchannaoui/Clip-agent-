...
function injectShell(){
  ...
  style.id = 'dcPhase4Styles'; style.textContent = css + billingCss + rangeChargeCss + v3Css + v3ProjectCss + clipToolsCss + scheduleKeepCss + topbarCleanCss + trialUxCss + v4DashboardCss + v4WorkspaceCss + v4CinematicCss; document.head.appendChild(style);
...
function renderHome(){
  ...
  panel.innerHTML = `
    <div class="dc-home-v4-cinema">
      <section class="dc-home-hero-g" data-tour="home-hero">
        <div class="dc-home-hero-copy">
          <div class="dc-v4-eyebrow"><i></i> DeenClipped V4 studio</div>
          <h1>Turn one long video into your next month of content.</h1>
          <p>Find the strongest moments, build beautiful vertical clips, and publish to your own channels from one premium creator workspace.</p>
          <div class="dc-stat-ribbon" aria-label="Workspace summary">${tinyStat(projects.length,'sources')}${tinyStat(clips.length,'clips')}${tinyStat(waiting,'to review')}${tinyStat(scheduled,'scheduled')}${tinyStat(posted,'published')}</div>
          <div class="dc-v3-actions"><button class="dc-btn" id="dcHeroCreate">Create new clips</button><button class="dc-btn secondary" data-dc-nav="review">${waiting?`Review ${waiting} clips`:'Open clip review'}</button><button class="dc-btn secondary" id="dcHeroTour">See the workflow</button></div>
          ${heroPlatformRail(d)}
        </div>
        <div class="dc-hero-stage" aria-label="Floating previews of your latest vertical clips"><div class="dc-v3-phone-wall">${heroThumbs(clips)}</div></div>
      </section>
      ...
      <section class="dc-home-import-g" data-tour="create-form">
        ...
        <button class="dc-btn secondary" id="dcPickVideo" type="button">Upload original</button>
      </section>

      <section class="dc-v4-feature-band" aria-label="Main workflow">
        ${flowCard('Import','YouTube link or original file',socialSvg('youtube'),'dcSourceYouTube')}
        ${flowCard('Review',waiting?`${waiting} clips waiting`:'Choose the strongest moments',ICON.review,'review')}
        ${flowCard('Style',selectedTemplate,ICON.style,'templates')}
        ${flowCard('Publish',connected?`${connected} channels connected`:'Connect your channels',ICON.publish,'publishing')}
      </section>

      <section class="dc-v4-action-shell">${v4NextAction(d,jobs,next,waiting,selectedTemplate)}</section>

      <section class="dc-home-command-g" data-tour="happening-now">
        <div class="dc-home-main-g">${liveFocusPanel(d,jobs,next,selectedTemplate)}<div class="dc-project-gallery-g"><div class="dc-simple-head"><div><h2>Continue creating</h2><p>Jump back into your latest sources and clips.</p></div><button class="dc-btn secondary" data-dc-nav="projects">View all projects</button></div><div class="dc-project-strip-g">${recentProjectsCinema(projects,clips)}</div></div></div>
        <aside class="dc-home-dock-g">${nextPostDock(next)}${platformDock(d)}${attentionDock(d)}</aside>
      </section>
    </div>`;
...
function renderReview(){
  ...
  panel.innerHTML=`<div class="dc-review-page-pro"><section class="dc-review-hero-pro"><div><span class="dc-review-kicker">V4 clip approval</span><h1>Pick winners fast.</h1><p>Compare the hook, posting copy and duration without opening every clip. Filter the queue, then edit only the moments worth publishing.</p></div><div class="dc-review-metrics-pro"><span><b>${allWaiting.length}</b><em>waiting</em></span><span><b>${strong}</b><em>strong hooks</em></span><span><b>${avg}</b><em>avg score</em></span></div></section><div class="dc-review-toolbar pro"><div class="dc-v4-filter-group"><button class="dc-v4-filter ${reviewFilter==='all'?'on':''}" data-review-filter="all">All ${allWaiting.length}</button><button class="dc-v4-filter ${reviewFilter==='strong'?'on':''}" data-review-filter="strong">Strong hooks ${strong}</button><button class="dc-v4-filter ${reviewFilter==='short'?'on':''}" data-review-filter="short">Under 60s</button><button class="dc-v4-filter ${reviewFilter==='verified'?'on':''}" data-review-filter="verified">Render verified</button><button class="dc-v4-filter ${reviewSort==='duration'?'on':''}" data-review-sort="${reviewSort==='duration'?'score':'duration'}">${reviewSort==='duration'?'Sort: shortest':'Sort: score'}</button></div><span class="spacer"></span><button class="dc-btn secondary" id="dcApproveVerified" ${!allWaiting.length?'disabled':''}>Approve verified</button><button class="dc-btn" id="dcScheduleAll" ${!waiting.length?'disabled':''}>Schedule visible</button></div><div class="dc-review-list pro">${waiting.length?waiting.map(reviewRow).join(''):`<div class="dc-review-empty-pro"><div><div class="dc-empty-icon">${ICON.review}</div><strong>${allWaiting.length?'No clips match this filter':'Clip review is clear'}</strong><p>${allWaiting.length?'Choose another filter to keep reviewing.':'New AI-selected clips will appear here with hook scores and captions.'}</p><button class="dc-btn" ${allWaiting.length?'data-review-filter="all"':'data-dc-nav="home"'}>${allWaiting.length?'Show every clip':'Import another lecture'}</button></div></div>`}</div></div>`;
  ...
  $$('[data-review-sort]',panel).forEach(button=>button.onclick=()=>{reviewSort=button.dataset.reviewSort;renderReview()});
  requestAnimationFrame(()=>animatePanel(panel));
}
...
const GUIDE_STEPS = [
  ...
  {view:'home',target:'[data-tour="create-form"]',title:'1. Paste your lecture link',copy:'Paste one YouTube or video URL here. For a public launch this will later sit behind login so every user has their own projects and accounts.'},
  ...
  {view:'schedule',target:'[data-dc-nav="schedule"]',title:'Schedule shows posting windows',copy:'Approved clips live here before posting. Users can post now, download, or check what is coming next.'},
...
function closeGuidedTour(){ try{localStorage.setItem('dc-guided-demo-complete','1')}catch{} $('#dcGuideLayer')?.remove(); }
...
function renderConnections(){
  ...
  panel.innerHTML=`<div class="dc-manage-page"><section class="dc-manage-hero"><div><span class="dc-manage-kicker">${ICON.social} V4 publishing hub</span><h1>Your channels, one approval flow.</h1><p>Connect each destination once. You still choose which clips are approved, scheduled or sent as drafts.</p></div><div class="dc-manage-metrics"><span><b>${connected}</b><em>connected</em></span><span><b>${enabled}</b><em>active</em></span><span><b>${d.directPublishingEnabled?'Ready':'Review'}</b><em>posting mode</em></span></div></section><div class="dc-manage-grid">${providers.map(connectionCard).join('')}</div><section class="dc-settings-panel"><h2>Choose active destinations</h2><p>Enabled channels become available when you approve or schedule a clip. TikTok uploads remain drafts for the creator to review in TikTok.</p><div class="dc-settings-form">${providers.map(p=>`<label class="dc-switch-row"><span><strong>${esc(providerTitle(p.provider))}</strong><span>${esc(p.account?.name||'No account selected')}</span></span><input type="checkbox" id="dcPub_${esc(p.provider)}" ${p.enabled?'checked':''} ${!p.connected?'disabled':''}></label>`).join('')}<button class="dc-btn wide" id="dcSavePublishing">Save active destinations</button></div></section></div>`;
