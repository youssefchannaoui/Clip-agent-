...
const NAV = [
  ['home','Home','home'], ['projects','Projects','projects'], ['review','Review clips','review'],
  ['editor','Editor','editor'], ['schedule','Publish','publish'], ['insights','Analytics','analytics']
];
const MANAGE = [['publishing','Social accounts','social'], ['music','Music','music'], ['automation','Settings','settings']];
...
.dc-video-canvas.is-manual-crop .dc-video-fg{cursor:grab}.dc-video-canvas.is-manual-crop.is-dragging .dc-video-fg{cursor:grabbing;transition:none}
...
function handleClick(event){
  ...
  const preset = event.target.closest('[data-quick-layout]'); if (preset) { applyQuickLayout(preset.dataset.quickLayout); return; }
...
function go(view){
  ...
  const labels = {
    ...
    review:['Review clips','Approve, edit or schedule clips'], editor:['Editor','CapCut-style clip editing workspace'],
    schedule:['Publish','Scheduling and platform delivery'], insights:['Analytics','Content and publishing performance'],
    publishing:['Social accounts','Connected destinations and permissions'], music:['Music','Nasheed library and audio level'],
...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    ...
    editor.draft={...clone(template),...(saved?.draft||{}),__clipId:clip.id};editor.draft.cropPositionX??=50;editor.draft.cropPositionY??=50;
...
async function loadCaptionWords(clip){
  try{
    ...
    if(Array.isArray(payload.words)&&payload.words.length){
      ...
      editor.captionSource=payload.exact?'whisper':'edited';editor.backendCaptionReady=true;if(!editor.dirty&&payload.transcript)editor.captionText=payload.transcript;
    }
...
function renderEditor(clip){
  ...
  panel.innerHTML=`<div class="dc-editor-header"><button class="dc-icon-btn dc-svg" id="dcEditorBack" title="Back to project">${ICON.back}</button><div class="dc-editor-title"><strong>${esc(clip.title||'Untitled clip')}</strong><span>${esc(clip.projectTitle||'Lecture')} · ${Math.round(clip.score||0)}/100 · ${editor.dirty?'Unsaved changes':'Saved'}</span></div><span class="dc-pill ${clip.status==='posted'?'good':'warn'}">${esc(statusName(clip.status))}</span><button class="dc-icon-btn dc-svg" id="dcUndo" title="Undo" ${editor.historyIndex<=0?'disabled':''}>${ICON.undo}</button><button class="dc-icon-btn dc-svg" id="dcRedo" title="Redo" ${editor.historyIndex>=editor.history.length-1?'disabled':''}>${ICON.redo}</button><button class="dc-btn secondary" id="dcSaveDraft">Save draft</button><button class="dc-btn" id="dcRenderClip">Render clip</button></div><div class="dc-editor-workspace"><nav class="dc-tool-rail">${toolButton('captions','Captions','captions')}${toolButton('canvas','Canvas','canvas')}${toolButton('style','Style','style')}${toolButton('audio','Audio','audio')}${toolButton('details','Details','details')}</nav><aside class="dc-tool-panel"><div class="dc-tool-head"><strong id="dcToolTitle">Captions</strong><span class="dc-pill ${editor.captionSource==='whisper'?'good':'warn'}" id="dcCaptionSource">${editor.captionSource==='whisper'?'Exact speech timing':editor.captionSource==='edited'?'Edited speech timing':'Estimated timing'}</span></div><div class="dc-tool-content" id="dcToolContent"></div></aside><main class="dc-canvas-area"><div class="dc-canvas-toolbar"><button class="dc-icon-btn dc-svg" id="dcPlayButton">${ICON.play}</button><span class="dc-timeline-time" id="dcCanvasTime">0:00 / ${formatClock(editor.trimOut)}</span><span class="spacer"></span><span class="dc-zoom">Source preview · render applies the exact same fill and framing</span></div><div class="dc-canvas-wrap"><div class="dc-video-canvas" id="dcVideoCanvas"><video id="dcEditorVideoBg" class="dc-video-layer dc-video-bg" src="${source}" preload="metadata" muted playsinline></video><video id="dcEditorVideo" class="dc-video-layer dc-video-fg" src="${source}" preload="metadata" playsinline></video><div class="dc-framing-guide"></div><span class="dc-layer-badge" id="dcLayerBadge">Video layer</span><div class="dc-caption-overlay" id="dcCaptionOverlay"></div><div class="dc-watermark" id="dcWatermark"></div><div class="dc-brand-line" id="dcBrandLine"></div><span class="dc-caption-status" id="dcCaptionStatus">Captions follow exact speech timing</span></div></div></main><section class="dc-timeline"><div class="dc-timeline-top"><span class="dc-timeline-time" id="dcTimelineTime">0:00.0</span><span class="spacer"></span><button class="dc-btn secondary" id="dcSavePreset">Save as preset</button></div><div class="dc-timeline-scroll" id="dcTimelineScroll"><div class="dc-ruler" id="dcRuler"></div><div class="dc-track-row"><div class="dc-track-label">Video</div><div class="dc-track-content"><div class="dc-video-block">${esc(clip.title||'Video')}</div></div></div><div class="dc-track-row"><div class="dc-track-label">Captions</div><div class="dc-track-content" id="dcCaptionTrack"></div></div><div class="dc-track-row"><div class="dc-track-label">Audio</div><div class="dc-track-content"><div class="dc-audio-block">${esc(clip.musicName||'Nasheed')}</div></div></div><div class="dc-playhead" id="dcPlayhead"></div></div></section></div>`;
  $('#dcEditorBack').onclick=()=>{selectedProjectId=clip.projectId;go('projects')};
  $('#dcUndo').onclick=undoEditor;$('#dcRedo').onclick=redoEditor;$('#dcSaveDraft').onclick=saveEditorDraft;$('#dcRenderClip').onclick=renderEditedClip;$('#dcSavePreset').onclick=saveEditorPreset;$('#dcPlayButton').onclick=togglePlayback;
...
function verifyEditorControls(){
  const required=['dcEditorBack','dcUndo','dcRedo','dcSaveDraft','dcRenderClip','dcSavePreset','dcPlayButton'];
...
function renderEditorTool(){
  const title={captions:'Captions',canvas:'Canvas',style:'Style',audio:'Audio',details:'Details'}[editor.tool];$('#dcToolTitle').textContent=title;$$('[data-editor-tool]').forEach(b=>b.classList.toggle('on',b.dataset.editorTool===editor.tool));
...
function captionTool(){
  return `<div class="dc-subtabs">${['styles','text','format','position'].map(x=>`<button type="button" class="${editor.captionTab===x?'on':''}" data-caption-tab="${x}">${x[0].toUpperCase()+x.slice(1)}</button>`).join('')}</div>${editor.captionTab==='styles'?captionStyles():editor.captionTab==='text'?captionTextPanel():editor.captionTab==='format'?captionFormat():captionPosition()}`;
}
function captionStyles(){
  const styles=[['viral','Viral pop','Active word grows'],['gold','Gold focus','DeenClipped highlight'],['clean','Clean','Simple phrases'],['arabic','Arabic','Readable Arabic script'],['bold','Bold','Large centred words'],['cinema','Cinema','Lower cinematic captions']];
  return `<div class="dc-section"><h3>Caption presets</h3><div class="dc-style-grid">${styles.map(([id,name,note])=>`<button class="dc-style-card" data-caption-style="${id}"><div class="dc-style-preview" style="${captionStylePreview(id)}">${id==='arabic'?'تذكير':'REMINDER'}</div><b>${name}</b><span>${note}</span></button>`).join('')}</div></div><div class="dc-caption-note">The preview now changes with playback instead of showing one permanent sample sentence. Final rendering uses the clip’s saved Whisper timing.</div>`;
}
...
function captionFormat(){
  ...
  return `<div class="dc-section"><h3>Behaviour</h3>${selectField('Caption mode','captionMode',[['dynamic-stack','Dynamic pop'],['word','Word highlight'],['phrase','Phrase captions']])}${selectField('Font','captionFont',[['Poppins','Poppins'],['Montserrat','Montserrat'],['Playfair Display','Playfair Display'],['Amiri','Amiri'],['Scheherazade New','Scheherazade Arabic'],['DejaVu Sans','DejaVu Sans']])}${rangeField('Font size','captionFontSize',24,140,1)}${rangeField('Words per caption','captionMaxWords',1,12,1)}${checkField('Uppercase captions','captionUppercase')}</div><div class="dc-section"><h3>Colours</h3><div class="dc-color-grid">${colorField('Text','captionPrimary')}${colorField('Highlight','captionHighlight')}${colorField('Outline','captionOutline')}${colorField('Background','captionBackground')}</div>${rangeField('Outline','captionOutlineWidth',0,14,1)}${rangeField('Background opacity','captionBackgroundOpacity',0,100,1)}${rangeField('Line spacing','captionLineHeight',.65,1.4,.05)}</div>`;
}
...
function canvasTool(){
  const [stateTitle,stateText]=framingStatusCopy(),fill=editor.draft.fitMode||'contain',manual=fill==='crop'&&!editor.draft.smartFramingEnabled;
  return `<div class="dc-canvas-explainer"><b>1. Choose the output frame.</b> Then choose how the imported lecture fills it. These controls only move or crop the video layer—they never move captions.</div><div class="dc-section"><h3>Output frame</h3>${selectField('Frame shape','__ratio',[['9:16','Vertical 9:16 · Shorts / Reels'],['4:5','Portrait 4:5'],['1:1','Square 1:1'],['16:9','Landscape 16:9']])}</div><div class="dc-section"><h3>How the source fits</h3><div class="dc-fill-options"><button type="button" aria-pressed="${fill==='contain'}" class="dc-fill-option ${fill==='contain'?'on':''}" data-fill-mode="contain"><span class="dc-fill-swatch contain"></span><b>Fit</b><span>Show the entire source</span></button><button type="button" aria-pressed="${fill==='blur'}" class="dc-fill-option ${fill==='blur'?'on':''}" data-fill-mode="blur"><span class="dc-fill-swatch blur"></span><b>Blur</b><span>Full source + blurred background</span></button><button type="button" aria-pressed="${fill==='crop'}" class="dc-fill-option ${fill==='crop'?'on':''}" data-fill-mode="crop"><span class="dc-fill-swatch crop"></span><b>Fill</b><span>Cover the frame and crop edges</span></button></div></div>${fill==='blur'?`<div class="dc-control-group"><strong>Blur background</strong><span>Only the background copy is blurred. The foreground source stays clear.</span>${rangeField('Blur strength','blurStrength',0,60,1)}</div>`:''}${fill==='crop'?`<div class="dc-control-group"><strong>Crop position</strong><span>${editor.draft.smartFramingEnabled?'AI moves the video crop around the fixed output frame. Caption position is untouched.':'Drag the video in the preview or use these controls.'}</span>${checkField('AI active-speaker framing','smartFramingEnabled')}${editor.draft.smartFramingEnabled?`${selectField('Fallback focus','smartFramingBias',[['auto','Automatic active speaker'],['left','Prefer left person'],['center','Prefer centre'],['right','Prefer right person']])}${rangeField('Crop zoom','smartFramingZoom',.75,1.35,.05)}${rangeField('Space around person','smartFramingPadding',.05,.45,.01)}${rangeField('Movement smoothing','smartFramingSmoothing',0,.95,.05)}<div class="dc-framing-state ${editor.framingStatus}"><i></i><div><strong>${esc(stateTitle)}</strong><span>${esc(stateText)}</span></div></div><div class="dc-inline-actions"><button type="button" class="dc-btn secondary" id="dcAnalyseFraming" ${editor.framingStatus==='analysing'?'disabled':''}>${editor.framingStatus==='analysing'?'Analysing…':'Analyse active speaker'}</button><button type="button" class="dc-btn secondary" id="dcResetFraming">Reset framing</button></div>`:`${rangeField('Horizontal crop','cropPositionX',0,100,1)}${rangeField('Vertical crop','cropPositionY',0,100,1)}<button type="button" class="dc-btn secondary" id="dcResetFraming" style="width:100%">Centre crop</button>`}</div>`:''}<div class="dc-section"><h3>One-click video layouts</h3><div class="dc-style-grid"><button type="button" class="dc-style-card" data-quick-layout="speaker"><div class="dc-layout-card-preview track"></div><b>Track speaker</b><span>Fill + active-speaker framing</span></button><button type="button" class="dc-style-card" data-quick-layout="full"><div class="dc-layout-card-preview fit"></div><b>Show full source</b><span>Fit without cropping</span></button><button type="button" class="dc-style-card" data-quick-layout="blur"><div class="dc-layout-card-preview blur"></div><b>Blur background</b><span>Full source over blur</span></button><button type="button" class="dc-style-card" data-quick-layout="fill"><div class="dc-layout-card-preview fill"></div><b>Fill centre</b><span>Cover frame, manual crop</span></button></div></div>${editor.captionSource==='fallback'?'<div class="dc-caption-warning">Caption timing is estimated because the caption backend is missing or unavailable. Run the complete Phase 4C installer—not only the activity file.</div>':''}`;
}
function styleTool(){
  return `<div class="dc-section"><h3>Video look</h3>${selectField('Filter','filterPreset',[['natural','Natural'],['crisp','Crisp'],['warm','Warm'],['cinematic','Cinematic'],['monochrome','Monochrome'],['custom','Custom']])}${rangeField('Brightness','brightness',-1,1,.05)}${rangeField('Contrast','contrast',.5,2,.05)}${rangeField('Saturation','saturation',0,3,.05)}${rangeField('Sharpen','sharpen',0,2,.05)}${rangeField('Vignette','vignette',0,1,.05)}</div><div class="dc-section"><h3>Branding</h3>${textField('Watermark','watermark')}${selectField('Watermark position','watermarkPosition',[['top-left','Top left'],['top-center','Top centre'],['top-right','Top right'],['bottom-left','Bottom left'],['bottom-center','Bottom centre'],['bottom-right','Bottom right']])}${rangeField('Watermark size','watermarkFontSize',12,90,1)}${rangeField('Watermark opacity','watermarkOpacity',0,100,1)}${colorField('Watermark colour','watermarkColor')}${checkField('Brand line','brandLineEnabled')}${colorField('Brand line colour','brandLineColor')}</div>`;
}
function audioTool(){
...
function resetFraming(){
  editor.draft.cropPositionX=50;editor.draft.cropPositionY=50;editor.framingPlan=null;editor.framingStatus='idle';editor.framingMessage='Framing reset';
...
function bindCanvasDrag(){
  const canvas=$('#dcVideoCanvas');if(!canvas)return;
  let drag=null;
  canvas.onpointerdown=event=>{
    if(editor.draft?.fitMode!=='crop'||editor.draft?.smartFramingEnabled)return;
    drag={x:event.clientX,y:event.clientY,startX:Number(editor.draft.cropPositionX??50),startY:Number(editor.draft.cropPositionY??50)};
    editor.canvasDragging=true;canvas.classList.add('is-dragging');canvas.setPointerCapture?.(event.pointerId);event.preventDefault();
  };
  canvas.onpointermove=event=>{
    if(!drag)return;
    const rect=canvas.getBoundingClientRect();editor.draft.cropPositionX=clamp(drag.startX-(event.clientX-drag.x)/Math.max(1,rect.width)*100,0,100);editor.draft.cropPositionY=clamp(drag.startY-(event.clientY-drag.y)/Math.max(1,rect.height)*100,0,100);
    applyFrameAtTime(editor.currentTime);$('[data-value-for="cropPositionX"]')?.replaceChildren(document.createTextNode(String(Math.round(editor.draft.cropPositionX))));$('[data-value-for="cropPositionY"]')?.replaceChildren(document.createTextNode(String(Math.round(editor.draft.cropPositionY))));
  };
  const finish=()=>{if(!drag)return;drag=null;editor.canvasDragging=false;canvas.classList.remove('is-dragging');markEditorDirty();pushHistory()};
...
function bindVideo(clip){
  ...
  video.onerror=()=>{
    ...
    video.src=authedUrl(clip.videoUrl);if(bg)bg.src=authedUrl(clip.videoUrl);
...
function updateEditorPreview(){
  ...
  overlay.className=`dc-caption-overlay ${d.captionPosition||'middle'} align-${d.captionHorizontal||'center'}`;
  overlay.style.fontFamily=d.captionFont||'Poppins';overlay.style.fontSize=`${clamp(Number(d.captionFontSize||96)/3.45,15,47)}px`;overlay.style.color=d.captionPrimary||'#fff';overlay.style.webkitTextStroke=`${Number(d.captionOutlineWidth||0)/3}px ${d.captionOutline||'#000'}`;overlay.style.textTransform=d.captionUppercase?'uppercase':'none';overlay.style.setProperty('--dc-cap-highlight',d.captionHighlight||'#d9b478');overlay.style.setProperty('--dc-cap-bg-color',hexAlpha(d.captionBackground||'#000000',clamp(Number(d.captionBackgroundOpacity||0)/100,0,1)));overlay.style.lineHeight=String(d.captionLineHeight||.9);
...
function updateCaptionAtTime(time){
  ...
  const index=words.findIndex(w=>time>=Number(w.start)&&time<Number(w.end)+hold);if(index<0){overlay.innerHTML='';return}
...
function captionSegments(){return speechGroups(editor.captionWords,Math.max(1,Number(editor.draft.captionMaxWords||4))).map(g=>({start:g.start,end:g.end,text:g.text}))}
...
async function saveEditorDraft(){
  ...
  }catch(error){notify(error.message,'bad')}finally{if(button){button.disabled=false;button.textContent='Save draft'}}
}
...
async function renderEditedClip(){
  ...
  }catch(error){notify(error.message,'bad')}finally{if(button){button.disabled=false;button.textContent='Render clip'}}
}
