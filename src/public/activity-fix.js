...
/* Dedicated full-screen editor. The dashboard shell is deliberately removed
   while editing so the canvas never has to fight the collapsible sidebar. */
body.dc-editor-mode{overflow:hidden!important}
body.dc-editor-mode #dcSidebar,body.dc-editor-mode #dcTopbar{display:none!important}
body.dc-editor-mode #app>.wrap{padding:0!important;height:100vh!important;overflow:hidden!important}
body.dc-editor-mode .main-col,body.dc-editor-mode #view-editor{height:100vh!important;max-width:none!important;margin:0!important}
body.dc-editor-mode #view-editor{position:fixed;inset:0;z-index:300;background:#09090b}
.dc-editor-v2{height:100%;display:grid;grid-template-rows:64px minmax(0,1fr) 170px;background:#09090b;color:var(--dc-text)}
.dc-editor-v2-top{display:flex;align-items:center;gap:10px;padding:0 14px;border-bottom:1px solid var(--dc-line);background:#111113}
.dc-editor-v2-top .dc-editor-title{flex:1}.dc-editor-actions{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.dc-editor-v2-main{min-height:0;display:grid;grid-template-columns:74px minmax(250px,310px) minmax(340px,1fr) minmax(250px,310px);overflow:hidden}
.dc-editor-v2 .dc-tool-rail{grid-row:auto;border-right:1px solid var(--dc-line)}
.dc-editor-v2 .dc-tool-panel{grid-row:auto;grid-column:auto;border-right:1px solid var(--dc-line)}
.dc-editor-stage{min-width:0;display:flex;flex-direction:column;background:#070708}
.dc-editor-stagebar{height:48px;display:flex;align-items:center;gap:8px;padding:0 12px;border-bottom:1px solid var(--dc-line)}
.dc-editor-stagebar .spacer{flex:1}.dc-editor-hint{font-size:10px;color:var(--dc-muted)}
.dc-editor-canvas-shell{flex:1;min-height:0;display:grid;place-items:center;padding:18px;overflow:hidden;background:radial-gradient(circle at 50% 40%,#1b1b20 0,#0b0b0d 52%,#070708 100%)}
.dc-editor-v2 .dc-video-canvas{height:min(66vh,690px);max-height:100%;touch-action:none;user-select:none;border-radius:8px;box-shadow:0 22px 70px #000;outline:1px solid #303037}
.dc-editor-v2 .dc-video-canvas video{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;transform-origin:center;pointer-events:none}
.dc-selection{position:absolute;z-index:12;border:1px solid rgba(255,255,255,.92);pointer-events:none;display:none}
.dc-selection.on{display:block}.dc-selection::before{content:attr(data-label);position:absolute;left:0;top:-24px;padding:3px 7px;border-radius:5px;background:#fff;color:#111;font-size:9px;font-weight:700;white-space:nowrap}
.dc-handle{position:absolute;width:22px;height:22px;border:3px solid #111;border-radius:50%;background:#fff;box-shadow:0 2px 10px #0008;pointer-events:auto;touch-action:none}
.dc-handle.br{right:-12px;bottom:-12px}.dc-handle.tl{left:-12px;top:-12px}.dc-handle.tr{right:-12px;top:-12px}.dc-handle.bl{left:-12px;bottom:-12px}
.dc-caption-overlay{left:auto!important;right:auto!important;top:auto!important;bottom:auto!important;transform:translate(-50%,-50%)!important;width:min(86%,860px);max-width:86%;padding:6px;cursor:move;pointer-events:auto!important;touch-action:none;overflow-wrap:anywhere}
.dc-caption-overlay.selected{outline:1px dashed rgba(255,255,255,.8);outline-offset:5px}
.dc-editor-inspector{min-width:0;border-left:1px solid var(--dc-line);background:#111113;overflow:auto;padding:14px}
.dc-inspector-card{padding:12px;border:1px solid var(--dc-line);border-radius:10px;background:#0c0c0e;margin-bottom:12px}
.dc-inspector-card h3{margin:0 0 9px;font-size:10px;color:var(--dc-accent2);text-transform:uppercase;letter-spacing:.08em}
.dc-inspector-row{display:grid;grid-template-columns:1fr 1fr;gap:8px}.dc-inspector-row .dc-btn{width:100%}
.dc-layer-list{display:flex;flex-direction:column;gap:6px}.dc-layer-button{display:flex;align-items:center;gap:9px;min-height:44px;padding:8px 10px;border:1px solid var(--dc-line);border-radius:8px;color:var(--dc-muted);text-align:left}.dc-layer-button.on{border-color:var(--dc-accent);background:rgba(217,180,120,.10);color:#fff}
.dc-layer-dot{width:9px;height:9px;border-radius:50%;background:var(--dc-accent)}
.dc-editor-v2 .dc-timeline{grid-column:auto;grid-row:auto;height:170px;border-top:1px solid var(--dc-line);padding:9px 14px}
.dc-editor-v2-bottom{min-height:0;display:grid;grid-template-columns:74px 310px 1fr 310px;background:var(--dc-panel);border-top:1px solid var(--dc-line)}
.dc-editor-v2-bottom .dc-timeline{grid-column:3/5;border-top:0}
.dc-safe-zone{position:absolute;inset:5% 7% 10%;z-index:9;border:1px dashed rgba(255,255,255,.18);pointer-events:none;border-radius:4px}
@media(max-width:1100px){.dc-editor-v2-main{grid-template-columns:64px 270px minmax(330px,1fr)}.dc-editor-inspector{display:none}.dc-editor-v2-bottom{grid-template-columns:64px 270px 1fr}.dc-editor-v2-bottom .dc-timeline{grid-column:3}}
@media(max-width:760px){body.dc-editor-mode{overflow:auto!important}.dc-editor-v2{height:auto;min-height:100dvh;grid-template-rows:auto auto 170px}.dc-editor-v2-top{min-height:64px;padding:8px;align-items:flex-start}.dc-editor-actions{justify-content:flex-end}.dc-editor-v2-main{grid-template-columns:1fr;grid-template-rows:58px auto minmax(560px,70dvh)}.dc-editor-v2 .dc-tool-rail{grid-row:1;flex-direction:row;overflow:auto;border-right:0;border-bottom:1px solid var(--dc-line)}.dc-editor-v2 .dc-tool-panel{grid-row:2;max-height:330px;border-right:0;border-bottom:1px solid var(--dc-line)}.dc-editor-stage{grid-row:3}.dc-editor-canvas-shell{padding:10px}.dc-editor-v2 .dc-video-canvas{height:min(66dvh,620px)}.dc-editor-v2-bottom{display:block}.dc-editor-v2-bottom .dc-timeline{height:170px}.dc-editor-v2-top .dc-editor-title span{display:none}}
...
function go(view){
  document.body.classList.toggle('dc-editor-mode', view === 'editor');
  currentView = view;
  ...
  const labels = {
    ...
    review:['Review','Edit or schedule clips that are ready'], editor:['Editor','Edit the selected clip'],
...
function renderHome(){
  ...
  panel.innerHTML = `<section class="dc-card dc-create-card"><h2>Create clips from a lecture</h2><p>Paste a source, choose the output, then follow every step under Happening now.</p><div class="dc-create-grid"><input id="dcCreateUrl" placeholder="Paste YouTube or video URL"><select id="dcCreateTemplate">${(d.templates||[]).map(t=>`<option value="${esc(t.id)}" ${t.id===d.selectedTemplate?.id?'selected':''}>${esc(t.name)}</option>`).join('')}</select><select id="dcCreateCount"><option>4</option><option selected>8</option><option>12</option><option>16</option></select><select id="dcCreateDuration"><option value="15,45">15–45 sec</option><option value="30,60" selected>30–60 sec</option><option value="45,90">45–90 sec</option></select><button class="dc-btn" id="dcGenerate">Generate clips</button></div></section><div class="dc-home-metrics">${metric(projects.length,'Lectures')}${metric(clips.length,'Generated clips')}${metric(waiting,'Waiting review')}${metric(posted,'Published')}</div><div class="dc-home-grid"><div class="dc-stack"><section class="dc-card dc-card-pad"><div class="dc-card-head"><div><h2>Happening now</h2><p>Generation, re-renders and uploads update live.</p></div><span class="dc-pill ${jobs.length?'warn':'good'}">${jobs.length?`${jobs.length} active`:'All clear'}</span></div><div class="dc-now-list">${jobs.length?jobs.map(jobRow).join(''):latestActivity(d)}</div></section><section class="dc-card dc-card-pad"><div class="dc-card-head"><div><h2>Recent projects</h2><p>Continue from the last lecture you worked on.</p></div><button class="dc-btn secondary" data-dc-nav="projects">View all</button></div><div class="dc-row-list">${recentProjects(projects,clips)}</div></section></div><div class="dc-stack"><section class="dc-card dc-card-pad"><div class="dc-card-head"><div><h2>Social accounts</h2><p>Connection status before publishing.</p></div><button class="dc-btn secondary" data-dc-nav="publishing">Manage</button></div><div class="dc-social-grid">${socialCards(d)}</div></section><section class="dc-card dc-card-pad"><div class="dc-card-head"><div><h2>Needs attention</h2><p>Only workflow blockers appear here.</p></div></div><div class="dc-row-list">${attentionRows(d)}</div></section><section class="dc-card dc-card-pad"><div class="dc-card-head"><div><h2>Upcoming posts</h2><p>${scheduled} clip${scheduled===1?'':'s'} currently approved or scheduled.</p></div><button class="dc-btn secondary" data-dc-nav="schedule">Open publish</button></div><div class="dc-row-list">${upcomingRows(clips)}</div></section></div></div>`;
...
function latestActivity(d){
  ...
  return logs.map(item=>`<div class="dc-list-row"><div class="dc-social-logo">✓</div><div class="dc-list-copy"><strong>${esc(typeof item==='string'?item:item.message||item.text||'Activity')}</strong><span>${item.at?formatRelative(item.at):'Latest activity'}</span></div></div>`).join('');
}
...
function projectCard(p,clips){
  const own=clips.filter(c=>c.projectId===p.id),thumb=own[0]?.thumbUrl,waiting=own.filter(c=>c.status==='waiting').length,posted=own.filter(c=>c.status==='posted').length;
  return `<article class="dc-project-card"><button class="dc-project-cover" data-open-project="${esc(p.id)}" type="button">${thumb?`<img src="${authedUrl(thumb)}">`:''}<span class="dc-project-status dc-pill ${p.status==='failed'?'bad':['queued','processing'].includes(p.status)?'warn':'good'}">${esc(statusName(p.status))}</span></button><div class="dc-project-body"><h3>${esc(p.title||'Untitled lecture')}</h3><p>${esc(p.stage||p.status)}${Number.isFinite(Number(p.progress))&&p.progress<100?` · ${Math.round(p.progress)}%`:''}</p><div class="dc-project-stats"><div class="dc-project-stat"><strong>${own.length}</strong><span>clips</span></div><div class="dc-project-stat"><strong>${waiting}</strong><span>review</span></div><div class="dc-project-stat"><strong>${posted}</strong><span>posted</span></div></div><div class="dc-project-actions"><button class="dc-btn" data-open-project="${esc(p.id)}">Open project</button>${p.status==='failed'?`<button class="dc-btn secondary" data-retry-project="${esc(p.id)}">Retry</button>`:`<button class="dc-btn secondary" data-more-project="${esc(p.id)}" ${!p.sourceReusable?'disabled':''}>More clips</button>`}</div></div></article>`;
}
function renderProjectDetail(panel,d){
  ...
  panel.innerHTML=`<div class="dc-project-detail-head"><button class="dc-icon-btn dc-svg" id="dcBackProjects">${ICON.back}</button><div class="dc-project-detail-copy"><h1>${esc(p.title||'Untitled lecture')}</h1><p>${clips.length} clips · ${esc(p.stage||p.status)}</p></div><button class="dc-btn secondary" data-more-project="${esc(p.id)}" ${!p.sourceReusable?'disabled':''}>Generate more</button><button class="dc-btn danger" data-delete-project="${esc(p.id)}">Delete project</button></div>${p.moreJob&&['queued','processing'].includes(p.moreJob.status)?`<div class="dc-card dc-card-pad" style="margin-bottom:12px"><div class="dc-now-row"><span class="dc-spinner"></span><div class="dc-now-main"><strong>${esc(p.moreJob.stage||'Generating more clips')}</strong><span>The saved lecture and transcript are being reused.</span><div class="dc-progress"><i style="width:${clamp(p.moreJob.progress,0,100)}%"></i></div></div><span class="dc-pill warn">${Math.round(p.moreJob.progress||0)}%</span></div></div>`:''}<div class="dc-clip-grid">${clips.length?clips.map(clipCard).join(''):`<div class="dc-empty"><strong>No clips yet</strong>${['queued','processing'].includes(p.status)?'Processing is still underway.':'Generate more clips from this lecture.'}</div>`}</div>`;
...
function clipCard(c){
  const nextAction=c.status==='waiting'
    ...
    :['approved','ready','publish_failed'].includes(c.status)
      ?`<button class="dc-btn secondary" data-post-clip="${esc(c.id)}">Post now</button>`:'';
  return `<article class="dc-clip-card"><button class="dc-clip-media" data-edit-clip="${esc(c.id)}" type="button"><img src="${authedUrl(c.thumbUrl)}"><span class="dc-score">${Math.round(c.score||0)}</span><span class="dc-duration">${formatDuration(c.durationMs)}</span></button><div class="dc-clip-body"><h3>${esc(c.title||'Untitled clip')}</h3><p>${statusName(c.status)}${c.scheduledAt?` · ${formatDate(c.scheduledAt)}`:''}</p><div class="dc-clip-actions"><button class="dc-btn" data-edit-clip="${esc(c.id)}">Edit</button>${nextAction||`<button class="dc-btn secondary" data-download-clip="${esc(c.id)}">Download</button>`}</div></div></article>`;
}
...
function renderReview(){
  ...
  panel.innerHTML=`<div class="dc-page-head"><div><h1>Review clips</h1><p>Edit the strongest moments before scheduling or posting.</p></div></div><div class="dc-review-toolbar"><span class="dc-pill ${waiting.length?'warn':'good'}">${waiting.length} waiting</span><span class="spacer"></span><button class="dc-btn secondary" id="dcApproveVerified" ${!waiting.length?'disabled':''}>Approve verified</button><button class="dc-btn" id="dcScheduleAll" ${!waiting.length?'disabled':''}>Schedule all</button></div><div class="dc-clip-grid">${waiting.length?waiting.map(c=>`${clipCard(c).replace('</div></div></article>',`<button class="dc-btn danger" data-delete-clip="${esc(c.id)}">Discard</button></div></div></article>`)}`).join(''):`<div class="dc-empty"><strong>Review queue is clear</strong>New generated clips will appear here.</div>`}</div>`;
...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    ...
    editor.draft={...clone(template),...(saved?.draft||{}),__clipId:clip.id};editor.draft.cropPositionX??=50;editor.draft.cropPositionY??=50;editor.draft.captionX??=50;editor.draft.captionY??=62;editor.draft.captionScale??=1;editor.draft.captionTimingOffsetMs??=-120;
...
function renderEditor(clip){
  const panel=$('#view-editor');if(!panel||!clip)return;
  panel.classList.add('dc-editor-page');document.body.classList.add('dc-editor-mode');
  const source=editorSourceUrl(clip);
  panel.innerHTML=`<div class="dc-editor-v2">
    <header class="dc-editor-v2-top">
      <button class="dc-icon-btn dc-svg" id="dcEditorBack" aria-label="Back to project">${ICON.back}</button>
      <div class="dc-editor-title"><strong>${esc(clip.title||'Untitled clip')}</strong><span>${esc(clip.projectTitle||'Lecture')} · ${editor.dirty?'Unsaved changes':'Saved'}</span></div>
      <div class="dc-editor-actions">
        <button class="dc-icon-btn dc-svg" id="dcUndo" aria-label="Undo" ${editor.historyIndex<=0?'disabled':''}>${ICON.undo}</button>
        <button class="dc-icon-btn dc-svg" id="dcRedo" aria-label="Redo" ${editor.historyIndex>=editor.history.length-1?'disabled':''}>${ICON.redo}</button>
        <button class="dc-btn secondary" id="dcSaveDraft">Save clip</button>
        <button class="dc-btn secondary" id="dcSavePreset">Save as template</button>
        <button class="dc-btn" id="dcRenderClip">Export video</button>
      </div>
    </header>
    <div class="dc-editor-v2-main">
      <nav class="dc-tool-rail">${toolButton('captions','Captions','captions')}${toolButton('canvas','Video','canvas')}${toolButton('style','Template','style')}${toolButton('audio','Audio','audio')}${toolButton('details','Post','details')}</nav>
      <aside class="dc-tool-panel"><div class="dc-tool-head"><strong id="dcToolTitle">Captions</strong><span class="dc-pill ${editor.captionSource==='whisper'?'good':'warn'}" id="dcCaptionSource">${editor.captionSource==='whisper'?'Auto-synced':'Timing estimated'}</span></div><div class="dc-tool-content" id="dcToolContent"></div></aside>
      <main class="dc-editor-stage">
        <div class="dc-editor-stagebar"><button class="dc-icon-btn dc-svg" id="dcPlayButton" aria-label="Play">${ICON.play}</button><span class="dc-timeline-time" id="dcCanvasTime">0:00 / ${formatClock(editor.trimOut)}</span><span class="spacer"></span><span class="dc-editor-hint">Tap a layer, then drag it. Use the white corner handle to resize.</span></div>
        <div class="dc-editor-canvas-shell"><div class="dc-video-canvas" id="dcVideoCanvas">
          <video id="dcEditorVideo" src="${source}" preload="metadata" playsinline></video>
          <div class="dc-safe-zone"></div>
          <div class="dc-caption-overlay" id="dcCaptionOverlay"></div>
          <div class="dc-watermark" id="dcWatermark"></div><div class="dc-brand-line" id="dcBrandLine"></div>
          <div class="dc-selection on" id="dcVideoSelection" data-label="Video"><button class="dc-handle br" id="dcResizeHandle" aria-label="Resize video"></button></div>
          <div class="dc-selection" id="dcCaptionSelection" data-label="Captions"><button class="dc-handle br" id="dcCaptionResize" aria-label="Resize captions"></button></div>
        </div></div>
      </main>
      <aside class="dc-editor-inspector">
        <div class="dc-inspector-card"><h3>Layers</h3><div class="dc-layer-list"><button class="dc-layer-button on" data-layer="video"><i class="dc-layer-dot"></i><span><b>Video</b><small>Drag and resize</small></span></button><button class="dc-layer-button" data-layer="captions"><i class="dc-layer-dot"></i><span><b>Captions</b><small>Always synced to speech</small></span></button></div></div>
        <div class="dc-inspector-card"><h3>Quick position</h3><div class="dc-inspector-row"><button class="dc-btn secondary" id="dcCenterLayer">Centre</button><button class="dc-btn secondary" id="dcResetLayer">Reset</button></div></div>
        <div class="dc-inspector-card"><h3>Template</h3><p class="dc-caption-note">Save this exact canvas, captions and styling as a reusable template. Saving the clip does not change the default template.</p><button class="dc-btn secondary" id="dcApplyPresetAll" style="width:100%;margin-top:9px">Apply template to old clips</button></div>
      </aside>
    </div>
    <div class="dc-editor-v2-bottom"><section class="dc-timeline"><div class="dc-timeline-top"><span class="dc-timeline-time" id="dcTimelineTime">0:00.0</span><span class="spacer"></span><span class="dc-caption-note">Caption timing loads automatically from Whisper</span></div><div class="dc-timeline-scroll" id="dcTimelineScroll"><div class="dc-ruler" id="dcRuler"></div><div class="dc-track-row"><div class="dc-track-label">Video</div><div class="dc-track-content"><div class="dc-video-block">${esc(clip.title||'Video')}</div></div></div><div class="dc-track-row"><div class="dc-track-label">Captions</div><div class="dc-track-content" id="dcCaptionTrack"></div></div><div class="dc-track-row"><div class="dc-track-label">Audio</div><div class="dc-track-content"><div class="dc-audio-block">${esc(clip.musicName||'Nasheed')}</div></div></div><div class="dc-playhead" id="dcPlayhead"></div></div></section></div>
  </div>`;
  $('#dcEditorBack').onclick=()=>{selectedProjectId=clip.projectId;go('projects')};
  $('#dcUndo').onclick=undoEditor;$('#dcRedo').onclick=redoEditor;$('#dcSaveDraft').onclick=saveEditorDraft;$('#dcSavePreset').onclick=saveEditorPreset;$('#dcApplyPresetAll').onclick=applyPresetToAllClips;$('#dcRenderClip').onclick=renderEditedClip;$('#dcPlayButton').onclick=togglePlayback;
  $$('[data-layer]').forEach(b=>b.onclick=()=>selectCanvasLayer(b.dataset.layer));
  $('#dcCenterLayer').onclick=()=>centreSelectedLayer();$('#dcResetLayer').onclick=()=>resetSelectedLayer();
  bindVideo(clip);bindCanvasDrag();renderEditorTool();updateEditorPreview();renderTimeline();selectCanvasLayer(editor.selectedLayer||'video');
...
function captionTool(){
  ...
  return `<div class="dc-sync-card"><div class="dc-sync-top"><i class="dc-sync-dot ${exact?'good':'busy'}"></i><div class="dc-sync-copy"><strong>${exact?'Captions automatically synced':'Loading exact speech timing'}</strong><span>${exact?'Whisper word timestamps are always used. No manual sync step is required.':'DeenClipped will use exact timestamps as soon as they are available.'}</span></div></div><details class="dc-advanced" style="margin-top:10px"><summary>Advanced timing correction</summary><div class="dc-field" style="margin-top:11px"><div class="dc-timing-readout"><span>Fine timing</span><b id="dcTimingLabel">${formatCaptionOffset(offset)}</b></div><input type="range" data-template-key="captionTimingOffsetMs" value="${offset}" min="-500" max="500" step="10"></div></details></div><div class="dc-subtabs">${['styles','text','format','position'].map(x=>`<button type="button" class="${editor.captionTab===x?'on':''}" data-caption-tab="${x}">${x[0].toUpperCase()+x.slice(1)}</button>`).join('')}</div>${editor.captionTab==='styles'?captionStyles():editor.captionTab==='text'?captionTextPanel():editor.captionTab==='format'?captionFormat():captionPosition()}`;
}
...
function applyCaptionPosition(value){const [vertical,horizontal]=value.split('-');editor.draft.captionPosition=vertical;editor.draft.captionHorizontal=horizontal;markEditorDirty();pushHistory();renderEditorTool();updateEditorPreview()}
...
function selectCanvasLayer(layer){
  editor.selectedLayer=layer==='captions'?'captions':'video';
  $$('#view-editor [data-layer]').forEach(b=>b.classList.toggle('on',b.dataset.layer===editor.selectedLayer));
  $('#dcVideoSelection')?.classList.toggle('on',editor.selectedLayer==='video');
  $('#dcCaptionSelection')?.classList.toggle('on',editor.selectedLayer==='captions');
  $('#dcCaptionOverlay')?.classList.toggle('selected',editor.selectedLayer==='captions');
}
function centreSelectedLayer(){
  if(editor.selectedLayer==='captions'){editor.draft.captionX=50;editor.draft.captionY=62}else{editor.draft.cropPositionX=50;editor.draft.cropPositionY=50}
  markEditorDirty();pushHistory();updateEditorPreview();renderEditorTool();
}
function resetSelectedLayer(){
  if(editor.selectedLayer==='captions'){editor.draft.captionX=50;editor.draft.captionY=62;editor.draft.captionScale=1}else{editor.draft.cropPositionX=50;editor.draft.cropPositionY=50;editor.draft.smartFramingZoom=1}
  markEditorDirty();pushHistory();updateEditorPreview();renderEditorTool();
}
function bindCanvasDrag(){
  const canvas=$('#dcVideoCanvas'),videoHandle=$('#dcResizeHandle'),captionHandle=$('#dcCaptionResize'),overlay=$('#dcCaptionOverlay');if(!canvas)return;
  let action=null;
  const begin=(event,type)=>{action={type,x:event.clientX,y:event.clientY,cx:Number(editor.draft.cropPositionX??50),cy:Number(editor.draft.cropPositionY??50),zoom:Number(editor.draft.smartFramingZoom||1),tx:Number(editor.draft.captionX??50),ty:Number(editor.draft.captionY??62),scale:Number(editor.draft.captionScale||1)};event.currentTarget?.setPointerCapture?.(event.pointerId);event.preventDefault();event.stopPropagation()};
  canvas.onpointerdown=e=>{if(e.target.closest('.dc-handle')||e.target.closest('#dcCaptionOverlay'))return;selectCanvasLayer('video');begin(e,'video-move')};
  overlay.onpointerdown=e=>{selectCanvasLayer('captions');begin(e,'caption-move')};
  videoHandle.onpointerdown=e=>{selectCanvasLayer('video');begin(e,'video-size')};
  captionHandle.onpointerdown=e=>{selectCanvasLayer('captions');begin(e,'caption-size')};
  canvas.onpointermove=e=>{
    if(!action)return;const rect=canvas.getBoundingClientRect(),dx=(e.clientX-action.x)/Math.max(1,rect.width)*100,dy=(e.clientY-action.y)/Math.max(1,rect.height)*100;
    if(action.type==='video-move'){editor.draft.smartFramingEnabled=false;editor.draft.fitMode='crop';editor.draft.cropPositionX=clamp(action.cx-dx,0,100);editor.draft.cropPositionY=clamp(action.cy-dy,0,100)}
    if(action.type==='video-size'){editor.draft.smartFramingEnabled=false;editor.draft.fitMode='crop';editor.draft.smartFramingZoom=clamp(action.zoom+(dx+dy)/80,.75,2.5)}
    if(action.type==='caption-move'){editor.draft.captionX=clamp(action.tx+dx,8,92);editor.draft.captionY=clamp(action.ty+dy,10,88)}
    if(action.type==='caption-size'){editor.draft.captionScale=clamp(action.scale+(dx+dy)/110,.55,1.8)}
    updateEditorPreview();
  };
  const finish=()=>{if(!action)return;action=null;markEditorDirty();pushHistory();renderEditorTool()};
  canvas.onpointerup=finish;canvas.onpointercancel=finish;
  canvas.onwheel=e=>{if(editor.selectedLayer!=='video')return;e.preventDefault();editor.draft.fitMode='crop';editor.draft.smartFramingEnabled=false;editor.draft.smartFramingZoom=clamp(Number(editor.draft.smartFramingZoom||1)+(e.deltaY<0?.05:-.05),.75,2.5);updateEditorPreview();markEditorDirty(false)};
}

function bindVideo(clip){
  const video=$('#dcEditorVideo');if(!video)return;
  ...
  const initialise=()=>{
    if(!editor.sourceFallback){try{video.currentTime=start}catch{}}
    renderTimeline();updateCaptionAtTime(0);applyFrameAtTime(0);
  };
  video.onloadedmetadata=initialise;
  video.onerror=()=>{
    if(editor.sourceFallback)return;
    editor.sourceFallback=true;editor.sourceBase=0;editor.sourceEnd=Math.max(.1,Number(clip.durationMs||0)/1000);
    video.src=authedUrl(clip.videoUrl);
    const status=$('#dcCaptionStatus');if(status)status.textContent='Rendered-video fallback · source file unavailable';
  };
  video.ontimeupdate=()=>{
    const local=clamp(video.currentTime-editor.sourceBase,0,editor.trimOut);editor.currentTime=local;updatePlayhead(local);updateCaptionAtTime(local);applyFrameAtTime(local);
    $('#dcCanvasTime').textContent=`${formatClock(local)} / ${formatClock(editor.trimOut)}`;$('#dcTimelineTime').textContent=formatClock(local,true);
    if(video.currentTime>=editor.sourceEnd-.02){video.pause();video.currentTime=editor.sourceBase}
  };
  video.onplay=()=>{editor.playing=true;$('#dcPlayButton').innerHTML=ICON.pause};
  video.onpause=()=>{editor.playing=false;$('#dcPlayButton').innerHTML=ICON.play};
...
function seekEditor(seconds){const video=$('#dcEditorVideo');if(!video)return;const local=clamp(seconds,0,editor.trimOut);video.currentTime=editor.sourceBase+local;if(bg)bg.currentTime=video.currentTime;editor.currentTime=local;updateCaptionAtTime(local);applyFrameAtTime(local)}
function updatePlayhead(time){const duration=editor.trimOut||1;const left=56+(time/duration)*($('#dcTimelineScroll')?.clientWidth-56||0);const head=$('#dcPlayhead');if(head)head.style.left=`${left}px`;applyFrameAtTime(time);$$('.dc-caption-block').forEach(b=>b.classList.toggle('active',time>=Number(b.dataset.captionStart)&&time<Number(b.dataset.captionEnd)))}

function updateEditorPreview(){
  const d=editor.draft,canvas=$('#dcVideoCanvas'),video=$('#dcEditorVideo'),overlay=$('#dcCaptionOverlay'),water=$('#dcWatermark'),line=$('#dcBrandLine');if(!canvas||!d)return;
  canvas.style.aspectRatio=`${d.width||1080}/${d.height||1920}`;canvas.dataset.fill=d.fitMode||'contain';
  ...
  const captionX=clamp(Number(d.captionX??50),8,92),captionY=clamp(Number(d.captionY??62),10,88),captionScale=clamp(Number(d.captionScale||1),.55,1.8);
  overlay.className=`dc-caption-overlay align-${d.captionHorizontal||'center'}${editor.selectedLayer==='captions'?' selected':''}`;
  overlay.style.left=`${captionX}%`;overlay.style.top=`${captionY}%`;overlay.style.fontFamily=d.captionFont||'Poppins';overlay.style.fontSize=`${clamp(Number(d.captionFontSize||96)/3.45*captionScale,13,64)}px`;overlay.style.color=d.captionPrimary||'#fff';overlay.style.webkitTextStroke=`${Number(d.captionOutlineWidth||0)/3}px ${d.captionOutline||'#000'}`;overlay.style.textShadow=`0 ${clamp(Number(d.captionShadow||0)/2,0,4)}px ${clamp(Number(d.captionShadow||0)*1.8,0,12)}px rgba(0,0,0,.58)`;overlay.style.textTransform=d.captionUppercase?'uppercase':'none';overlay.style.setProperty('--dc-cap-highlight',d.captionHighlight||'#fff');overlay.style.setProperty('--dc-cap-highlight-font',d.captionHighlightFont||'DejaVu Serif');overlay.style.setProperty('--dc-cap-arabic-font',d.captionArabicFont||'Amiri');overlay.style.setProperty('--dc-cap-highlight-style',d.captionHighlightItalic===false?'normal':'italic');overlay.style.setProperty('--dc-cap-highlight-glow',`${clamp(Number(d.captionHighlightGlow||0)/2.5,0,14)}px`);overlay.style.setProperty('--dc-cap-bg-color',hexAlpha(d.captionBackground||'#000000',clamp(Number(d.captionBackgroundOpacity||0)/100,0,1)));overlay.style.lineHeight=String(d.captionLineHeight||.9);
  const capSel=$('#dcCaptionSelection');if(capSel){const rect=overlay.getBoundingClientRect(),cRect=canvas.getBoundingClientRect();capSel.style.left=`${rect.left-cRect.left}px`;capSel.style.top=`${rect.top-cRect.top}px`;capSel.style.width=`${rect.width}px`;capSel.style.height=`${rect.height}px`}
  const vidSel=$('#dcVideoSelection');if(vidSel){vidSel.style.inset='4px'}
  ...
  line.style.display=d.brandLineEnabled?'block':'none';line.style.background=d.brandLineColor||'#d9b478';
}
...
function renderTimeline(){
  const video=$('#dcEditorVideo'),duration=video?.duration||editor.trimOut||1,ruler=$('#dcRuler'),track=$('#dcCaptionTrack');if(!ruler||!track)return;
  ruler.innerHTML=Array.from({length:6},(_,i)=>`<span style="left:${i*20}%">${formatClock(duration*i/5)}</span>`).join('');
  track.innerHTML=captionSegments().map(s=>{const left=100*s.start/duration,width=Math.max(.8,100*(s.end-s.start)/duration);return `<button class="dc-caption-block" data-caption-start="${s.start}" data-caption-end="${s.end}" style="left:${left}%;width:${width}%">${esc(s.text)}</button>`}).join('');
...
async function saveEditorDraft(){
  const clip=currentClip();if(!clip)return;const button=$('#dcSaveDraft');button.disabled=true;button.textContent='Saving…';
  try{
    await savePostDetails(false);
    const before=clip.transcript||'';
    await callApi(`/api/clips/${encodeURIComponent(clip.id)}`,{method:'PATCH',body:JSON.stringify({transcript:editor.captionText})});
    saveEditorLocal();await refreshData();
    const updated=currentClip();
    if(editor.captionText!==before&&updated&&updated.transcript!==editor.captionText)throw new Error('Caption backend patch is not installed. Run apply-phase4.mjs from the package.');
    editor.dirty=false;clearEditorLocal();notify('Editor draft saved');renderEditor(updated||clip);
