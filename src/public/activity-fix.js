...
const MANAGE = [];
const CUSTOM = new Set(['home','projects','review','editor','schedule','publishing','templates','brand','lab','music','automation','insights','admin']);
...
function injectShell(){
  ...
  const main = $('.main-col');
  if (main) {
    for (const name of ['home','projects','review','editor','publishing','templates','brand','lab','music','automation','insights','admin']) {
      if (!$(`#view-${name}`)) {
...
function openBillingModal(){
  ...
  const planCards=sortPlans.map(id=>{
    ...
    const label=isFree?(isCurrent?'Current free plan':'Continue free'):isCurrent?'Current plan':enabled?`Start ${trialDays}-day trial`:'Stripe price needed';
    const featureCopy=[`${minutesFor(tokens)} source minutes`,...(Array.isArray(plan.features)?plan.features:[])];
  ...
  const tabs=sortPlans.map(id=>`<button id="dcBillingTab_${esc(id)}" type="button" role="tab" aria-controls="dcBillingPlan_${esc(id)}" aria-selected="${id==='monthly'?'true':'false'}" class="${id==='monthly'?'active':''}" data-billing-tab="${esc(id)}">${esc(id[0].toUpperCase()+id.slice(1))}${id==='weekly'?'<small>Cheapest</small>':id==='monthly'?'<small>Popular</small>':''}</button>`).join('');
  layer.innerHTML=`<section class="dc-billing-card" role="dialog" aria-modal="true" aria-labelledby="dcBillingTitle"><div class="dc-billing-head"><div><span class="dc-billing-kicker">${ICON.tokens} Tokens & billing</span><h2 id="dcBillingTitle">Choose the rhythm that fits you.</h2><p>Start free, pay weekly, or save with a longer plan.</p></div><button class="dc-billing-close" id="dcBillingClose" type="button" aria-label="Close">×</button></div><div class="dc-billing-tabs" role="tablist" aria-label="Billing plan">${tabs}</div><div class="dc-wallet-strip"><div><span>${esc(usageLabel)}</span><strong>${esc(usageValue)} <small>${esc(usageSub)}</small></strong></div><div><span>Current allowance</span><strong>${esc(allowanceCopy)}</strong></div><div><span>Usage</span><strong>${esc(spentCopy)}</strong></div><div><span>Source rate</span><strong>${esc(rate)} <small>token/min</small></strong></div><span class="dc-status-pill ${unlimited?'warn':trial.active?'good':''}">${esc(unlimited?'Admin mode':trialStatus)}</span><div class="dc-wallet-progress" aria-hidden="true"><i style="width:${pct}%"></i></div></div><div class="dc-plan-grid">${planCards}</div><div class="dc-billing-explainer"><span><b>Estimate first</b>You approve the token cost before processing.</span><span><b>Charged exactly</b>Only the source duration used is charged.</span><span><b>Edits stay free</b>Template updates and rerenders use no tokens.</span></div><div class="dc-billing-foot"><div class="dc-billing-note"><i class="${billingReady?'':'warn'}"></i><span>${billingReady?'Secure checkout powered by Stripe.':'Stripe checkout is being configured.'}</span></div><button class="dc-btn secondary" id="dcBillingPortal" type="button" ${cur.stripeCustomerId?'':'disabled'}>Manage billing</button></div></section>`;
  document.body.append(layer);
  $('#dcBillingClose').onclick=()=>layer.remove();
  layer.addEventListener('click',event=>{if(event.target===layer)layer.remove()});
  $$('[data-billing-tab]',layer).forEach(tab=>tab.addEventListener('click',()=>{
    const selected=tab.dataset.billingTab;
    $$('[data-billing-tab]',layer).forEach(item=>{const active=item.dataset.billingTab===selected;item.classList.toggle('active',active);item.setAttribute('aria-selected',String(active))});
    $$('[data-billing-plan]',layer).forEach(card=>{const active=card.dataset.billingPlan===selected;card.classList.toggle('active',active);card.hidden=!active});
  }));
...
.dc-nav-button[data-dc-nav=projects] .dc-nav-icon{color:#72b7ff}.dc-nav-button[data-dc-nav=review] .dc-nav-icon{color:#edb763}.dc-nav-button[data-dc-nav=schedule] .dc-nav-icon{color:#70d7a2}.dc-nav-button[data-dc-nav=publishing] .dc-nav-icon{color:#66d5ff}.dc-nav-button[data-dc-nav=editor] .dc-nav-icon{color:#b796ff}.dc-nav-button[data-dc-nav=brand] .dc-nav-icon{color:#efc976}.dc-nav-button[data-dc-nav=lab] .dc-nav-icon{color:#b590ff}.dc-nav-button[data-dc-nav=music] .dc-nav-icon{color:#ff8fc9}.dc-nav-button[data-dc-nav=insights] .dc-nav-icon{color:#71d6a0}.dc-nav-button[data-dc-nav=automation] .dc-nav-icon{color:#8eb9ff}
