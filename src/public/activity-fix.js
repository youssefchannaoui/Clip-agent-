...
function renderHome(){
  ...
  panel.innerHTML = `<section class="dc-v3-hero" data-tour="home-hero"><div><div class="dc-v3-kicker">${ICON.scissors} V3 workspace</div><h1 class="dc-v3-title">Create clean dawah clips without staring at a wall of text.</h1><p class="dc-v3-copy">Paste a lecture, pick the template, review thumbnails, then publish. The app now shows the workflow visually so new users know exactly what to do next.</p><div class="dc-v3-actions"><button class="dc-btn" id="dcHeroCreate">Start with YouTube</button><button class="dc-btn secondary" id="dcHeroTour">Run guided demo</button><button class="dc-btn secondary" data-dc-nav="projects">View projects</button></div><div class="dc-v3-pipeline">${workflowStep('youtube','Import','Paste YouTube or upload a video')}${workflowStep('captions','Clip Review','Approve the best moments')}${workflowStep('publish','Publish','Schedule or post to platforms')}</div></div><div class="dc-v3-phone-wall" aria-label="Recent clip thumbnails">${heroThumbs(clips)}</div></section><div class="dc-v3-source-row" data-tour="source-options"><button class="dc-v3-source" type="button" id="dcSourceYouTube"><span class="dc-v3-platform youtube">${ICON.youtube}</span><strong>YouTube import</strong><span>Paste a lecture and begin processing.</span></button><button class="dc-v3-source" type="button" data-dc-nav="templates"><span class="dc-v3-platform template">${ICON.style}</span><strong>${esc(selectedTemplate)}</strong><span>Current default look for every render.</span></button><button class="dc-v3-source" type="button" data-dc-nav="publishing"><span class="dc-v3-platform publish">${ICON.social}</span><strong>Publishing setup</strong><span>Connect TikTok, Instagram, Facebook and YouTube.</span></button></div><section class="dc-card dc-create-card v3" data-tour="create-form"><div class="dc-create-grid"><input id="dcCreateUrl" placeholder="Paste YouTube or video URL"><select id="dcCreateTemplate" data-tour="template-picker">${(d.templates||[]).map(t=>`<option value="${esc(t.id)}" ${t.id===d.selectedTemplate?.id?'selected':''}>${esc(t.name)}</option>`).join('')}</select><select id="dcCreateCount"><option>4</option><option selected>8</option><option>12</option><option>16</option></select><select id="dcCreateDuration"><option value="15,45">15–45 sec</option><option value="30,60" selected>30–60 sec</option><option value="45,90">45–90 sec</option></select><button class="dc-btn" id="dcGenerate" data-tour="generate-button">Generate clips</button></div></section><div class="dc-home-metrics v3">${metric(projects.length,'Lectures')}${metric(clips.length,'Generated clips')}${metric(waiting,'Clip review')}${metric(scheduled,'Scheduled')}${metric(posted,'Published')}</div><div class="dc-home-dashboard"><section class="dc-card dc-card-pad v3-card dc-dashboard-card" data-tour="happening-now"><div class="dc-card-head"><div><h2>Happening now</h2><p>Live activity only.</p></div><span class="dc-pill ${jobs.length?'warn':'good'}">${jobs.length?`${jobs.length} active`:'All clear'}</span></div><div class="dc-row-list compact">${jobs.length?jobs.slice(0,4).map(jobRow).join(''):latestActivity(d)}</div></section><section class="dc-card dc-card-pad v3-card dc-dashboard-card" data-tour="platform-cards"><div class="dc-card-head"><div><h2>Platform connections</h2><p>Publishing readiness.</p></div><button class="dc-btn secondary" data-dc-nav="publishing">Manage</button></div><div class="dc-social-grid clean">${socialCards(d)}</div></section><section class="dc-card dc-card-pad v3-card dc-dashboard-card"><div class="dc-card-head"><div><h2>Recent projects</h2><p>Continue where you left off.</p></div><button class="dc-btn secondary" data-dc-nav="projects">View all</button></div><div class="dc-row-list compact">${recentProjects(projects,clips)}</div></section><section class="dc-card dc-card-pad v3-card dc-dashboard-card"><div class="dc-card-head"><div><h2>Upcoming posts</h2><p>${scheduled} ready or scheduled.</p></div><button class="dc-btn secondary" data-dc-nav="schedule">Open publish</button></div><div class="dc-row-list compact">${upcomingRows(clips)}</div></section><section class="dc-card dc-card-pad v3-card dc-dashboard-card"><div class="dc-card-head"><div><h2>Needs attention</h2><p>Only blockers.</p></div></div><div class="dc-row-list compact">${attentionRows(d)}</div></section></div>`;
...
function upcomingRows(clips){
  ...
  if(!list.length) return `<div class="dc-empty"><strong>No clips scheduled</strong>Clip Review, then schedule them for publishing.</div>`;
...
function projectCard(p,clips){
  const own=clips.filter(c=>c.projectId===p.id),thumb=own[0]?.thumbUrl,waiting=own.filter(c=>c.status==='waiting').length,scheduled=own.filter(c=>['approved','scheduled','publishing'].includes(c.status)).length,posted=own.filter(c=>c.status==='posted').length;
  const title=p.title||p.url||'Untitled lecture';
  const failed=p.status==='failed', busy=['queued','processing'].includes(p.status);
  const badgeClass=failed?'bad':busy?'warn':'good';
  const stage=failed?(p.error||'Processing failed'):busy?(p.stage||'Processing'):waiting?'Clips need review':'Ready';
  return `<article class="dc-project-card"><button class="dc-project-cover" data-open-project="${esc(p.id)}" type="button" aria-label="Open ${esc(title)}">${thumb?`<img src="${authedUrl(thumb)}" alt="${esc(title)} thumbnail">`:`<div class="dc-v3-phone-empty">${ICON.projects}</div>`}<span class="dc-project-status dc-pill ${badgeClass}">${esc(statusName(p.status))}</span></button><div class="dc-project-body"><h3>${esc(title)}</h3><div class="dc-project-meta-pills"><span class="dc-pill ${failed?'bad':busy?'warn':'good'}">${esc(stage)}</span></div><div class="dc-project-stats"><div class="dc-project-stat"><strong>${own.length}</strong><span>clips</span></div><div class="dc-project-stat"><strong>${waiting}</strong><span>review</span></div><div class="dc-project-stat"><strong>${scheduled}</strong><span>scheduled</span></div></div><div class="dc-project-actions"><button class="dc-btn" data-open-project="${esc(p.id)}">Open project</button>${failed?`<button class="dc-btn secondary" data-retry-project="${esc(p.id)}">Retry</button>`:`<button class="dc-btn secondary" data-more-project="${esc(p.id)}" ${!p.sourceReusable?'disabled':''}>More clips</button>`}</div></div></article>`;
}
...
function renderSidebarLive(){
  ...
  const waiting=clips.filter(c=>c.status==='waiting').length;
  const scheduled=clips.filter(c=>Number(c.scheduledAt)>Date.now()).length;
  const next=clips.filter(c=>Number(c.scheduledAt)>Date.now()).sort((a,b)=>a.scheduledAt-b.scheduledAt)[0];
  const current=jobs[0];
  const body=current?`<div class="dc-mini-job"><span class="dc-mini-job-icon">${current.kind==='publish'?ICON.publish:current.kind==='render'?ICON.editor:ICON.scissors}</span><div><strong>${esc(current.title)}</strong><span>${esc(current.stage)}${Number.isFinite(current.progress)?` · ${Math.round(current.progress)}%`:''}</span></div></div>`:`<div class="dc-mini-job"><span class="dc-mini-job-icon">${ICON.check}</span><div><strong>All clear</strong><span>${next?`Next ${formatDate(next.scheduledAt)}`:'No active job'}</span></div></div>`;
  box.innerHTML=`<div class="dc-sidebar-live-head"><div><strong>Happening now</strong><br><span>${current?'Live workflow':'Workflow status'}</span></div><i class="dc-live-orb ${current?'busy':''}"></i></div>${body}<div class="dc-sidebar-status-pills"><div class="dc-side-pill"><b>${waiting}</b><span>review</span></div><div class="dc-side-pill"><b>${scheduled}</b><span>scheduled</span></div></div><div class="dc-sidebar-live-foot"><button class="dc-btn secondary" data-dc-nav="review">Open Clip Review</button></div>`;
}
