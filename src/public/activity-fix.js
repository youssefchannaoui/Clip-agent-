...
.dc-video-fg{z-index:1;transition:object-position .34s cubic-bezier(.2,.75,.25,1),transform .24s ease,opacity .18s ease;clip-path:inset(0);transform-origin:center center}
...
function bindVideo(clip){
  ...
  video.onerror=()=>{
    ...
    video.pause();video.removeAttribute('src');video.load();if(bg){bg.pause();bg.removeAttribute('src');bg.load();bg.style.display='none'}
    const status=$('#dcCaptionStatus');if(status)status.textContent='Original source unavailable · preview disabled to prevent duplicate burned-in captions';
    notify('The original source video is unavailable, so the editor preview was disabled instead of showing a duplicate rendered clip.','bad');
  };
  video.ontimeupdate=()=>{
