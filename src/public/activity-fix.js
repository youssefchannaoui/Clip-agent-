...
.dc-video-canvas.is-manual-crop .dc-video-fg{cursor:grab}.dc-video-canvas.is-manual-crop.is-dragging .dc-video-fg{cursor:grabbing;transition:none}.dc-resize-handle{display:none;position:absolute;right:10px;bottom:10px;z-index:12;width:48px;height:48px;border-radius:50%;border:3px solid #fff;background:rgba(0,0,0,.78);box-shadow:0 2px 10px #000a;touch-action:none;cursor:nwse-resize}.dc-resize-handle::before{content:'↗';display:grid;place-items:center;width:100%;height:100%;font-size:21px;color:#fff}.dc-video-canvas.is-manual-crop .dc-resize-handle{display:block}.dc-video-canvas.is-resizing .dc-video-fg{transition:none}
...
function bindCanvasDrag(){
  const canvas=$('#dcVideoCanvas'),handle=$('#dcResizeHandle');if(!canvas)return;
  let drag=null,resize=null;
  canvas.onpointerdown=event=>{
    if(editor.draft?.fitMode!=='crop'||editor.draft?.smartFramingEnabled)return;
    if(event.target.closest('#dcResizeHandle'))return;
  ...
  const finishDrag=()=>{if(!drag)return;drag=null;editor.canvasDragging=false;canvas.classList.remove('is-dragging');markEditorDirty();pushHistory()};
  canvas.onpointerup=finishDrag;canvas.onpointercancel=finishDrag;
  if(handle){
    handle.onpointerdown=event=>{if(editor.draft?.fitMode!=='crop'||editor.draft?.smartFramingEnabled)return;resize={x:event.clientX,y:event.clientY,start:Number(editor.draft.smartFramingZoom||1)};canvas.classList.add('is-resizing');handle.setPointerCapture?.(event.pointerId);event.stopPropagation();event.preventDefault()};
    handle.onpointermove=event=>{if(!resize)return;const rect=canvas.getBoundingClientRect(),delta=((event.clientX-resize.x)+(event.clientY-resize.y))/Math.max(1,rect.width+rect.height)*2;editor.draft.smartFramingZoom=clamp(resize.start+delta,.75,2.5);applyFrameAtTime(editor.currentTime)};
    const finishResize=()=>{if(!resize)return;resize=null;canvas.classList.remove('is-resizing');markEditorDirty();pushHistory();renderEditorTool()};
    handle.onpointerup=finishResize;handle.onpointercancel=finishResize;
  }
  canvas.onwheel=event=>{if(editor.draft?.fitMode!=='crop'||editor.draft?.smartFramingEnabled)return;event.preventDefault();editor.draft.smartFramingZoom=clamp(Number(editor.draft.smartFramingZoom||1)+(event.deltaY<0?.05:-.05),.75,2.5);applyFrameAtTime(editor.currentTime);markEditorDirty(false)};
}
...
async function saveEditorPreset(){
  ...
  finally{if(button){button.disabled=false;button.textContent='Use for all new clips'}}
}
async function applyPresetToAllClips(){
  ...
  }catch(e){notify(e.message,'bad')}finally{if(button){button.disabled=false;button.textContent='Apply to all old clips'}}
}
