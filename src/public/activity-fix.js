...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    ...
    editor.dirty=Boolean(saved);editor.history=[];editor.historyIndex=-1;pushHistory(true);
...
async function loadCaptionWords(clip){
  try{
    ...
    if(Array.isArray(payload.words)&&payload.words.length){editor.captionWords=payload.words.map(w=>({start:Number(w.start),end:Number(w.end),word:String(w.word||'').trim()})).filter(w=>w.word&&w.end>w.start);editor.captionSource=payload.exact?'whisper':'edited';if(!editor.dirty&&payload.transcript)editor.captionText=payload.transcript;}
...
function renderEditor(clip){
  ...
  bindVideo(clip);renderEditorTool();updateEditorPreview();renderTimeline();
}
...
function captionTextPanel(){
  const groups=captionSegments().slice(0,20);
  return `<div class="dc-section"><h3>Edit caption transcript</h3><textarea class="dc-caption-editor" id="dcCaptionText">${esc(editor.captionText)}</textarea><div class="dc-caption-note" style="margin-top:7px">Saving transcript changes requires the included Phase 4 backend patch. Edited words are redistributed across the current clip duration.</div></div><div class="dc-section"><h3>Caption segments</h3><div class="dc-caption-list">${groups.map(g=>`<button class="dc-caption-line" data-caption-start="${g.start}"><span>${formatClock(g.start)}</span><b>${esc(g.text)}</b></button>`).join('')}</div></div>`;
}
...
function canvasTool(){
  return `<div class="dc-section"><h3>Canvas</h3>${selectField('Aspect ratio','__ratio',[['9:16','Vertical 9:16'],['4:5','Portrait 4:5'],['1:1','Square 1:1'],['16:9','Landscape 16:9']])}${selectField('Fill mode','fitMode',[['contain','Fit entire video'],['blur','Blurred background'],['crop','Fill / crop']])}${checkField('Smart framing','smartFramingEnabled')}${selectField('Speaker focus','smartFramingBias',[['auto','Automatic'],['left','Left'],['center','Centre'],['right','Right']])}${rangeField('Blur strength','blurStrength',0,60,1)}</div><div class="dc-section"><h3>Quick layouts</h3><div class="dc-style-grid"><button class="dc-style-card" data-quick-layout="speaker"><b>Speaker focus</b><span>Crop and follow speaker</span></button><button class="dc-style-card" data-quick-layout="full"><b>Full frame</b><span>Show the complete source</span></button><button class="dc-style-card" data-quick-layout="cinematic"><b>Cinematic</b><span>Film look and low captions</span></button><button class="dc-style-card" data-quick-layout="minimal"><b>Minimal</b><span>Clean layout and branding</span></button></div></div>`;
}
function styleTool(){
...
function bindToolInputs(){
  ...
  $('#dcCaptionText')?.addEventListener('input',event=>{
    editor.captionText=event.target.value;editor.captionWords=approximateWords(editor.captionText,Math.max(.1,editor.trimOut-editor.trimIn));editor.captionSource='edited';markEditorDirty();updateCaptionAtTime(editor.currentTime);renderTimeline();debouncedHistory();
  });
...
function applyQuickLayout(id){
  if(id==='speaker')Object.assign(editor.draft,{fitMode:'crop',smartFramingEnabled:true,smartFramingBias:'auto',filterPreset:'natural'});
  if(id==='full')Object.assign(editor.draft,{fitMode:'contain',smartFramingEnabled:false,filterPreset:'natural'});
  if(id==='cinematic')Object.assign(editor.draft,{fitMode:'crop',filterPreset:'cinematic',vignette:.35,captionPosition:'bottom',captionFont:'Playfair Display'});
  if(id==='minimal')Object.assign(editor.draft,{fitMode:'contain',filterPreset:'natural',captionMode:'phrase',captionHorizontal:'center',watermark:'',brandLineEnabled:false});
...
function bindVideo(clip){
  ...
  video.onloadedmetadata=()=>{editor.trimOut=Math.min(editor.trimOut||video.duration,video.duration||editor.trimOut);renderTimeline();updateCaptionAtTime(video.currentTime)};
  video.ontimeupdate=()=>{editor.currentTime=video.currentTime;updatePlayhead(video.currentTime);updateCaptionAtTime(video.currentTime);$('#dcCanvasTime').textContent=`${formatClock(video.currentTime)} / ${formatClock(video.duration||editor.trimOut)}`;$('#dcTimelineTime').textContent=formatClock(video.currentTime,true);if(video.currentTime>=editor.trimOut){video.pause();video.currentTime=editor.trimIn}};
  ...
  $('#dcTimelineScroll').addEventListener('click',event=>{if(event.target.closest('[data-caption-start]'))return;const content=event.target.closest('.dc-track-content,.dc-ruler');if(!content)return;const rect=content.getBoundingClientRect();seekEditor(clamp((event.clientX-rect.left)/rect.width,0,1)*(video.duration||editor.trimOut))});
}
function togglePlayback(){const video=$('#dcEditorVideo');if(!video)return;video.paused?video.play():video.pause()}
function seekEditor(seconds){const video=$('#dcEditorVideo');if(!video)return;video.currentTime=clamp(seconds,0,video.duration||editor.trimOut);updateCaptionAtTime(video.currentTime)}
function updatePlayhead(time){const duration=$('#dcEditorVideo')?.duration||editor.trimOut||1;const left=56+(time/duration)*($('#dcTimelineScroll')?.clientWidth-56||0);const head=$('#dcPlayhead');if(head)head.style.left=`${left}px`;$$('.dc-caption-block').forEach(b=>b.classList.toggle('active',time>=Number(b.dataset.captionStart)&&time<Number(b.dataset.captionEnd)))}
...
function updateEditorPreview(){
  ...
  canvas.style.aspectRatio=`${d.width||1080}/${d.height||1920}`;
...
function updateCaptionAtTime(time){
  ...
  const index=words.findIndex(w=>time>=w.start&&time<w.end);if(index<0){overlay.innerHTML='';return}
  const mode=editor.draft.captionMode||'dynamic-stack',max=Math.max(1,Number(editor.draft.captionMaxWords||4));let html='';
  if(mode==='phrase'){
    const start=Math.floor(index/max)*max,group=words.slice(start,start+max);html=`<span class="dc-caption-bg">${group.map((w,i)=>`<span class="dc-caption-word ${start+i===index?'active':''}">${esc(w.word)}</span>`).join(' ')}</span>`;
  }else if(mode==='word'){
    const start=Math.floor(index/max)*max,group=words.slice(start,start+max);html=`<span class="dc-caption-bg">${group.map((w,i)=>`<span class="dc-caption-word ${start+i===index?'active':''}">${esc(w.word)}</span>`).join(' ')}</span>`;
  }else{
    const start=Math.floor(index/max)*max,group=words.slice(start,index+1);html=`<span class="dc-caption-bg">${group.map((w,i)=>`<span class="dc-caption-stack-line ${i===group.length-1?'active':''}">${esc(w.word)}</span>`).join('')}</span>`;
  }
...
function captionSegments(){
  const words=editor.captionWords,max=Math.max(1,Number(editor.draft.captionMaxWords||4)),segments=[];
  for(let i=0;i<words.length;i+=max){const group=words.slice(i,i+max);segments.push({start:group[0].start,end:group.at(-1).end,text:group.map(w=>w.word).join(' ')})}return segments;
}
