...
body.dc-app .dc-plan-grid{align-items:stretch;gap:16px;padding:0 30px 18px}
...
function openBillingModal(){
  ...
  const planCards=sortPlans.map(id=>{
    ...
    return `<article class="dc-plan-card ${featured?'featured':''} ${isCurrent?'current':''}">${featured?'<div class="dc-plan-featured-label">Most popular</div>':''}<div class="dc-plan-content"><div class="dc-plan-top"><div><h3>${esc(plan.name||id)}</h3><span>${esc(id==='weekly'?'Flexible starter':id==='monthly'?'Creator favourite':'Best long-term value')}</span></div>${!featured||isCurrent?`<span class="badge">${esc(isCurrent?'Active':plan.badge||'Plan')}</span>`:''}</div><div class="dc-plan-price">${esc(plan.priceLabel||'Price shown at checkout')}<span> / ${esc(intervalWord[id]||id)}</span></div><div class="tokens">${esc(tokens)} <span>tokens included</span></div><p>${esc(plan.description||'Token allowance for clipping lectures.')}</p><div class="dc-plan-divider"></div><div class="dc-plan-features">${featureCopy.map(item=>`<span>${esc(item)}</span>`).join('')}</div><button class="dc-btn ${isCurrent?'secondary':''}" data-billing-checkout="${esc(id)}" ${enabled&&!isCurrent?'':'disabled'}>${esc(label)}</button></div></article>`;
  }).join('');
