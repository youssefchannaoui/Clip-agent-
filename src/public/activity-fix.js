...
function bindCanvasDrag(){
  ...
  const ensureManual=()=>{if(!editor.draft)return false;if(editor.draft.fitMode!=='crop'){editor.draft.fitMode='crop';editor.draft.cropPositionX??=50;editor.draft.cropPositionY??=50;}switchToManualFraming('Manual crop selected by dragging or resizing the video');canvas.classList.add('is-manual-crop','video-selected');return true};
  video.onpointerdown=event=>{
    if(event.button!==undefined&&event.button!==0)return;
    if(!ensureManual())return;
    $('#dcCaptionOverlay')?.classList.remove('is-selected');
    ...
    canvas.classList.add('is-dragging','is-manual-crop','video-selected');video.setPointerCapture?.(event.pointerId);event.stopPropagation();event.preventDefault();
  };
...
function bindCaptionDrag(){
  ...
  overlay.onpointerdown=event=>{
    ...
    canvas.classList.remove('video-selected');
  ...
  overlay.onclick=event=>{overlay.classList.add('is-selected');canvas.classList.remove('video-selected');event.stopPropagation();};
  canvas.addEventListener('pointerdown',event=>{if(event.target.closest('#dcCaptionOverlay'))return;overlay.classList.remove('is-selected');if(event.target.closest('#dcEditorVideo,#dcResizeHandle'))canvas.classList.add('video-selected')});
}
