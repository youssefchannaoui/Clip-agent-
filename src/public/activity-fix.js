...
function captionTool(){
  // No clean plate means the captions in the video are permanent. Saying so is
  // the whole panel; offering styles and positions that cannot be applied is
  // what made this screen feel broken.
  if(!editorHasCleanSource(currentClip())){
    return `<div class="dc-sync-card is-locked"><div class="dc-sync-top"><i class="dc-sync-dot warn"></i><div class="dc-sync-copy"><strong>Captions are part of this video</strong><span>This clip’s original footage is no longer available, so its captions are baked into the picture and cannot be restyled or moved. Everything else here still works.</span></div></div><p class="dc-caption-note">To get editable captions, re-import the lecture and let DeenClipped process it. Clips made that way keep their clean footage, so captions stay separate from the video.</p></div>`;
  }
