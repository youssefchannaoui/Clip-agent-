...
async function approveClip(id){
  try{const result=await callApi(`/api/clips/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status:'approved'})});notify(result.clip?.scheduledLabel?`Approved · next slot ${result.clip.scheduledLabel}`:'Clip approved and scheduled');await refreshData();renderReview()}catch(e){notify(e.message,'bad')}
}
...
function publishingSlotBoard(d,projects){
  ...
  return `<div class="dc-publish-slot-tools"><div><strong>Automatic slot queue</strong><span>${esc(zone)} · approve a clip and it takes the next open time</span></div><div class="sched-range" aria-label="Schedule range"><button class="${publishingSlotDays===1?'on':''}" data-publish-slot-days="1">Today</button><button class="${publishingSlotDays===5?'on':''}" data-publish-slot-days="5">5 days</button><button class="${publishingSlotDays===14?'on':''}" data-publish-slot-days="14">2 weeks</button></div></div><div class="schedule-board">${rows.map(({slot,clip})=>{
    ...
    const actions=clip?`<button class="dc-btn" data-post-clip="${esc(clip.id)}">Post now</button><button class="dc-btn secondary" data-download-clip="${esc(clip.id)}">Download</button><button class="dc-btn secondary" data-edit-style-clip="${esc(clip.id)}">Preview</button>`:`<span class="slot-status">${isNext?'Next available':'Available'}</span>`;
...
function renderPublishingWorkspace(){
  ...
  if(!['slots','queue','scheduled','posted'].includes(publishingQueueTab))publishingQueueTab='slots';
