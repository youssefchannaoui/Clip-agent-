...
function injectShell(){
  ...
  style.id = 'dcPhase4Styles'; style.textContent = css + v3Css + projectDetailFixCss; document.head.appendChild(style);
...
function renderHome(){
  ...
  const failed = projects.filter(p=>p.status==='failed'||p.error).length + clips.filter(c=>c.status==='publish_failed').length;
  const selectedTemplate = d.selectedTemplate?.name || 'No template selected';
  panel.innerHTML = `<section class="dc-v3-hero dc-v3-hero-focus" data-tour="home-hero"><div><div class="dc-v3-kicker">${ICON.scissors} V3 command centre</div><h1 class="dc-v3-title">Import. Review. Edit. Publish.</h1><p class="dc-v3-copy">A cleaner workspace for turning long Islamic lectures into short clips. Start from YouTube, approve the best moments, then post to connected platforms.</p><div class="dc-v3-actions"><button class="dc-btn" id="dcHeroCreate">Start with YouTube</button><button class="dc-btn secondary" data-dc-nav="projects">Open projects</button><button class="dc-btn secondary" id="dcHeroTour">Guided demo</button></div></div><div class="dc-v3-phone-wall" aria-label="Recent clip thumbnails">${heroThumbs(clips)}</div></section><section class="dc-card dc-create-card v3" data-tour="create-form"><div class="dc-create-grid"><input id="dcCreateUrl" placeholder="Paste YouTube or video URL"><select id="dcCreateTemplate" data-tour="template-picker">${(d.templates||[]).map(t=>`<option value="${esc(t.id)}" ${t.id===d.selectedTemplate?.id?'selected':''}>${esc(t.name)}</option>`).join('')}</select><select id="dcCreateCount"><option>4</option><option selected>8</option><option>12</option><option>16</option></select><select id="dcCreateDuration"><option value="15,45">15–45 sec</option><option value="30,60" selected>30–60 sec</option><option value="45,90">45–90 sec</option></select><button class="dc-btn" id="dcGenerate" data-tour="generate-button">Generate clips</button></div></section><div class="dc-home-metrics v3 dc-home-metrics-tight">${metric(projects.length,'Lectures')}${metric(clips.length,'Clips')}${metric(waiting,'Need review')}${metric(scheduled,'Scheduled')}${metric(posted,'Posted')}</div><div class="dc-home-command-grid"><button class="dc-home-command platform" type="button" id="dcSourceYouTube"><i>${socialSvg('youtube')}</i><strong>Import lecture</strong><span>YouTube link or video URL</span></button><button class="dc-home-command" type="button" data-dc-nav="review"><i>${ICON.review}</i><strong>Clip Review</strong><span>${waiting} waiting for approval</span></button><button class="dc-home-command" type="button" data-dc-nav="editor"><i>${ICON.style}</i><strong>${esc(shortText(selectedTemplate,26))}</strong><span>Current default look</span></button><button class="dc-home-command" type="button" data-dc-nav="publishing"><i>${ICON.social}</i><strong>Connections</strong><span>YouTube, TikTok, Meta</span></button></div><div class="dc-home-layout-v3d"><div class="dc-stack"><section class="dc-card dc-card-pad v3-card dc-now-compact" data-tour="happening-now"><div class="dc-card-head"><div><h2>Happening now</h2><p>Live jobs and latest result only.</p></div><span class="dc-pill ${jobs.length?'warn':failed?'bad':'good'}">${jobs.length?`${jobs.length} live`:failed?`${failed} issue${failed===1?'':'s'}`:'All clear'}</span></div><div class="dc-now-list dc-compact-card-list">${jobs.length?jobs.slice(0,3).map(jobRow).join(''):latestActivity(d)}</div></section><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Recent projects</h2><p>Jump back into a lecture.</p></div><button class="dc-btn secondary" data-dc-nav="projects">View all</button></div><div class="dc-row-list dc-compact-card-list">${recentProjects(projects,clips)}</div></section></div><div class="dc-stack"><section class="dc-card dc-card-pad v3-card" data-tour="platform-cards"><div class="dc-card-head"><div><h2>Platform connections</h2><p>Posting readiness.</p></div><button class="dc-btn secondary" data-dc-nav="publishing">Manage</button></div><div class="dc-social-grid">${socialCards(d)}</div></section><div class="dc-home-split-mini"><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Upcoming</h2><p>${scheduled} scheduled.</p></div><button class="dc-btn secondary" data-dc-nav="schedule">Open</button></div><div class="dc-row-list dc-compact-card-list">${upcomingRows(clips)}</div></section><section class="dc-card dc-card-pad v3-card"><div class="dc-card-head"><div><h2>Attention</h2><p>Fix blockers fast.</p></div></div><div class="dc-row-list dc-compact-card-list">${attentionRows(d)}</div></section></div></div></div>`;
...
function jobRow(j){ return `<div class="dc-now-row"><span class="dc-spinner"></span><div class="dc-now-main"><strong>${esc(shortText(j.title,52))}</strong><span>${esc(shortText(j.stage,44))}</span>${Number.isFinite(j.progress)?`<div class="dc-progress"><i style="width:${clamp(j.progress,0,100)}%"></i></div>`:''}</div><span class="dc-pill warn">${Number.isFinite(j.progress)?`${Math.round(j.progress)}%`:'Live'}</span></div>`; }
function latestActivity(d){
  const logs = (d.log || []).slice(0,3);
  if (!logs.length) return `<div class="dc-empty"><strong>Idle</strong>New jobs appear here.</div>`;
...
function recentProjects(projects, clips){
  ...
  return list.map(p=>{const own=clips.filter(c=>c.projectId===p.id);const thumb=own[0]?.thumbUrl;const failed=p.status==='failed'||p.error;return `<button class="dc-list-row" data-open-project="${esc(p.id)}" type="button">${thumb?`<img class="dc-thumb" src="${authedUrl(thumb)}">`:'<div class="dc-thumb"></div>'}<div class="dc-list-copy"><strong>${esc(shortText(projectDisplayTitle(p),58))}</strong><span>${own.length} clips · ${failed?'Needs retry':statusName(p.status)}</span></div><span class="dc-pill ${failed?'bad':p.status==='processing'?'warn':'good'}">${failed?'Issue':statusName(p.status)}</span></button>`}).join('');
}
...
function renderProjects(){
  ...
  document.body.classList.toggle('dc-project-open', Boolean(selectedProjectId));
  panel.classList.toggle('dc-project-detail-view', Boolean(selectedProjectId));
  panel.classList.add('dc-projects-full');
...
function renderProjectDetail(panel,d){
  ...
  const thumb=clips.find(c=>c.thumbUrl)?.thumbUrl;
  const statusClass=p.status==='failed'?'bad':['queued','processing'].includes(p.status)?'warn':'good';
  panel.innerHTML=`<div class="dc-project-detail-v4"><section class="dc-detail-cover"><button class="dc-icon-btn dc-svg" id="dcBackProjects" aria-label="Back to projects">${ICON.back}</button><div class="dc-detail-cover-thumb">${thumb?`<img src="${authedUrl(thumb)}">`:`<div class="dc-project-placeholder">${ICON.projects}<span>Lecture</span></div>`}</div><div class="dc-detail-title"><h1>${esc(projectDisplayTitle(p))}</h1><p>${clips.length} clips · ${waiting} need review · ${scheduled} scheduled · ${posted} posted</p></div><span class="dc-pill ${statusClass}">${esc(statusName(p.status))}</span><div class="dc-detail-actions"><button class="dc-btn secondary" data-more-project="${esc(p.id)}" ${!p.sourceReusable?'disabled':''}>Generate more</button><button class="dc-btn secondary" data-dc-nav="review">Clip Review</button><button class="dc-btn danger" data-delete-project="${esc(p.id)}">Delete project</button></div></section><div class="dc-detail-stats">${metric(clips.length,'Total clips')}${metric(waiting,'Need review')}${metric(scheduled,'Scheduled')}${metric(posted,'Posted')}</div>${p.status==='failed'||p.error?`<div class="dc-project-error v4"><strong>Processing issue</strong><span>${esc(shortError(p.error||p.stage))}</span><button class="dc-btn secondary" data-retry-project="${esc(p.id)}">Retry</button></div>`:''}${p.moreJob&&['queued','processing'].includes(p.moreJob.status)?`<div class="dc-card dc-card-pad"><div class="dc-now-row"><span class="dc-spinner"></span><div class="dc-now-main"><strong>${esc(p.moreJob.stage||'Generating more clips')}</strong><span>Reusing saved lecture and transcript.</span><div class="dc-progress"><i style="width:${clamp(p.moreJob.progress,0,100)}%"></i></div></div><span class="dc-pill warn">${Math.round(p.moreJob.progress||0)}%</span></div></div>`:''}<div class="dc-detail-filterbar"><select><option>All clips</option><option>Waiting review</option><option>Scheduled</option><option>Posted</option></select><select><option>Highest score</option><option>Newest first</option><option>Longest</option></select><span class="dc-pill">${clips.length} clips</span><span class="spacer"></span></div><div class="dc-clip-grid dc-project-clip-grid">${clips.length?clips.map(c=>clipCard(c,{detail:true})).join(''):`<div class="dc-empty dc-empty-full"><strong>No clips yet</strong>${['queued','processing'].includes(p.status)?'Processing is still underway.':'Generate more clips from this lecture.'}</div>`}</div></div>`;
...
function clipCard(c,opts={}){
  ...
  return `<article class="dc-clip-card v3-clip-card v4"><button class="dc-clip-media" data-edit-style-clip="${esc(c.id)}" type="button">${c.thumbUrl?`<img src="${authedUrl(c.thumbUrl)}">`:`<div class="dc-project-placeholder">${ICON.play}<span>Clip</span></div>`}<span class="dc-score">${Math.round(c.score||0)}</span><span class="dc-duration">${formatDuration(c.durationMs)}</span><span class="dc-clip-state dc-pill ${c.status==='posted'?'good':c.status==='waiting'?'warn':c.status==='publish_failed'?'bad':''}">${statusName(c.status)}</span><button class="dc-btn danger dc-clip-delete-float" data-delete-clip="${esc(c.id)}" type="button">Delete</button></button><div class="dc-clip-body"><h3>${esc(title)}</h3><p>${esc(sub)}</p><div class="dc-clip-actions"><button class="dc-btn" data-edit-style-clip="${esc(c.id)}">Edit style</button><button class="dc-btn secondary" data-edit-video-clip="${esc(c.id)}">Edit video</button>${canPost?`<button class="dc-btn secondary" data-post-clip="${esc(c.id)}">Post now</button>`:canSchedule?`<button class="dc-btn secondary" data-schedule-clip="${esc(c.id)}">Schedule</button>`:`<button class="dc-btn secondary" data-download-clip="${esc(c.id)}">Download</button>`}<button class="dc-btn secondary" data-download-clip="${esc(c.id)}">Download</button></div></div></article>`;
}
...
async function ensureEditor(){
  ...
  if(editor.clipId!==clip.id){
    const requestedTool=editor.pendingTool||editor.tool||'captions'; const requestedTab=editor.pendingCaptionTab||editor.captionTab||'styles'; editor.loading=true; editor.clipId=clip.id; editor.tool=requestedTool;editor.captionTab=requestedTool==='captions'?requestedTab:'styles';editor.pendingTool='';editor.pendingCaptionTab='';editor.search='';
...
function renderEditor(clip){
  ...
  $('#dcUndo').onclick=undoEditor;$('#dcRedo').onclick=redoEditor;$('#dcSaveDraft').onclick=saveEditorDraft;$('#dcRenderClip').onclick=renderEditedClip;$('#dcPlayButton').onclick=togglePlayback;$('#dcOpenCaptionText')?.addEventListener('click',()=>{editor.tool='captions';editor.captionTab='text';renderEditorTool();$('#dcCaptionText')?.focus();});
...
function renderSidebarLive(){
  ...
  box.innerHTML=`<div class="dc-sidebar-live-head"><div><strong>Happening now</strong><br><span>${jobs.length?'Live job':'Workflow status'}</span></div><i class="dc-live-orb ${jobs.length?'busy':''}"></i></div>${currentRow}${nextRow}<div class="dc-sidebar-live-foot"><button class="dc-btn secondary" data-dc-nav="review">Review</button><button class="dc-btn secondary" data-dc-nav="schedule">Publish</button></div>`;
}
