...
async function initClip(clip){
  ...
  S.draft.width??=1080;S.draft.height??=1920;S.draft.fitMode??='crop';S.draft.cropPositionX??=50;S.draft.cropPositionY??=50;
...
function manualPanel(){
  return `<div style="margin-top:10px">${range('Horizontal position','cropPositionX',0,100,1,S.draft.cropPositionX??50)}
  ${range('Vertical position','cropPositionY',0,100,1,S.draft.cropPositionY??50)}
  <button class="dce-btn secondary" id="dceCentreCrop" style="width:100%">Centre crop</button></div>`;
}
function aiPanel(){
...
function setMode(value){
  ...
  if(value!=='fill'){S.draft.smartFramingEnabled=false;S.framing=null;S.framingState='idle'}
...
function updatePreview(){
  ...
  q('#dceModeBadge').textContent=d.fitMode==='crop'?(d.smartFramingEnabled?'Fill · AI speaker focus':'Fill · drag to reposition'):d.fitMode==='blur'?'Blur · full source':'Fit · full source';
...
async function syncCaptions(){
  ...
  try{await loadCaptions(true);S.offsetMs=-140;S.draft.captionTimingOffsetMs=S.offsetMs;markDirty();pushHistory();renderInspector();updateCaption(localTime());toast('Captions synced to saved speech timing');setPreviewStatus('Exact captions ready')}
...
async function analyseFraming(){
  ...
  }catch(e){S.framingState='error';renderInspector();updatePreview();toast(e.message,true);setPreviewStatus('Tracking failed',true)}
}
