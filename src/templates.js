...
const DEFAULTS = Object.freeze({
  ...
  promoBarStartSec: 3,
  promoBarSeconds: 3,
  // THE ATMOSPHERE: weather over the picture. Youssef, 4 Sept 2026: "add
  // another thing so they can add sencery or layover, so layour can be dark
  // with rain drops but still the video of couese."
  //
  // Two independent halves, and they are separate fields because they are
  // separate decisions: `overlayDarken` dims the picture (which is also what
  // makes a caption pop on bright footage) and `overlayEffect` puts particles
  // over it. "Dark with rain" is both; either alone is a real choice.
  //
  // OFF by default. It costs render time -- measured at roughly two to three
  // times the video-filter stage -- and it is a look, not a correction.
  overlayEffect: 'none',
  overlayIntensity: 55,
  overlayDarken: 0,
...
const ENUMS = {
  ...
  // Twelve graded looks, every one of them rendered and looked at before it
  // was kept -- two of the first draft (a teal that was indistinguishable from
  // cinematic, and a "night" that did not read as night) were rebuilt because
  // the frame said so, not because the numbers looked wrong.
  //
  // Three are black and white on purpose: Youssef asked for "black and white"
  // and the three are genuinely different pictures -- monochrome is flat,
  // noir is hard, silver is soft with lifted blacks.
  filterPreset: ['natural', 'crisp', 'vivid', 'warm', 'cinematic', 'teal',
    'faded', 'night', 'monochrome', 'noir', 'silver', 'sepia', 'custom'],
  // Particles drifting over the picture. Generated by the renderer from a
  // deterministic noise field -- there is no artwork to ship, so nothing here
  // can go missing on a box that has not pulled an asset.
...
export const NUMBER_RANGES = {
  ...
  // 10 is the floor rather than 0: an effect switched on and drawing nothing
  // is a control that does nothing, and 'none' is how it is turned off.
  // Darken stops at 80 because the instruction was "but still the video of
  // course" -- past that the picture is gone and only the captions remain.
  overlayIntensity: [10, 100], overlayDarken: [0, 80],
