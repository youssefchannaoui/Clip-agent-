...
export function plansPage(user, { error = '', info = '', returnTo = '/' } = {}) {
  ...
  // The obvious way in for someone who just wants to try it. A new account is
  // 302'd straight to this page, so the free path must be the FIRST decision
  // on screen, not a footnote below the token shop. Shown only while the
  ...
  /*
   * THE FREE CARD IS THE PRIMARY ACTION ONLY WHILE THERE IS SOMETHING FREE
   * LEFT TO TAKE.
   *
   * It used to be shown to anyone on the free PLAN, expired or not -- so an
   * account whose seven days had gone met a gold "Continue with free tokens"
   * button offering forty tokens it could not give them. That is the loudest
   * thing on the page telling somebody to do the one thing that will not work.
   *
   * The two states are also the two jobs this page has. On day one nobody buys
   * before they have made a clip, so the honest primary action is "get in and
   * try it" and the plans are there to be READ. Once the window has closed the
   * decision is real, so the free card steps back to a sentence and the plans
   * lead.
   */
  const grant = cur.grant || {};
  const freeSpent = Boolean(cur.freeTrial && cur.freeTrial.expired);
  const freeOpen = onFree && !freeSpent && !grant.ended;
  const heroGo = !freeOpen ? ''
    : `<div class="hero-go"><form method="post" action="/billing/continue-free"><input type="hidden" name="returnTo" value="${returnValue}"><button type="submit">Start free — no card</button></form><span>${esc(String(freeTokens))} tokens · ${esc(String(trialDays || 7))} days · import, review, templates, scheduling and publishing, all of it. Upgrade whenever you need more minutes.</span></div>`;
  const freeDone = (!onFree || freeOpen) ? ''
    : `<section class="free-done"><strong>${grant.ended ? `Your ${esc(cur.grantTierName || 'Pro')} access has ended.` : 'Your free days are up.'}</strong><span>Everything you made is still here — your clips, your schedule and your connected channels. Choose a plan below and it all picks up where it left off.</span></section>`;

  /*
   * The code box, and it matters MORE here than in the app: a new account is
   * sent to this page the moment it signs up, so somebody who was DM'd a code
   * meets this screen before they ever reach the dashboard. Youssef, 7 Sept
   * 2026: "the tester code thing should also come on this page as well. This
   * page is probably more important."
   *
   * A plain form post, deliberately: this page carries NO script -- the period
   * switch is CSS radios for the same reason, and a test asserts the page has
   * no script tag. So the round trip through the server is the whole mechanism.
   */
  const codeBox = cur.unlimited ? '' : `<section class="code-row"${grant.active ? ' data-on="1"' : ''}><div class="code-say"><strong>${grant.active ? `${esc(cur.grantTierName || 'Pro')} access is running` : 'Have a code?'}</strong><span>${grant.active
    ? `${esc(String(Math.max(0, Number(grant.daysLeft || 0))))} day${Number(grant.daysLeft) === 1 ? '' : 's'} left, with every ${esc(cur.grantTierName || 'Pro')} feature on. Choose a plan before it ends to carry straight on.`
    : 'If we sent you a tester or launch code, put it in here to unlock it on this account.'}</span></div><form method="post" action="/billing/redeem-code"><input type="hidden" name="returnTo" value="${returnValue}"><input name="code" autocomplete="off" spellcheck="false" placeholder="DEEN-XXXXXX" aria-label="Your code"><button type="submit">Apply code</button></form></section>`;
  ...
  :root{color-scheme:dark;--bg:#070607;--bg1:#0d0c0e;--panel:#131115;--line:rgba(245,241,232,.09);--line2:rgba(245,241,232,.17);--text:#f5f1e8;--muted:#a8a196;--muted2:#736d63;--gold:#d9b66f;--gold2:#f1d18e;--green:#6fce9e;--red:#ef6b7a;--serif:"Fraunces",Georgia,"Times New Roman",serif;--mono:ui-monospace,SFMono-Regular,Menlo,monospace;--ease:cubic-bezier(.22,.6,.18,1)}*{box-sizing:border-box}body{margin:0;min-height:100vh;background:radial-gradient(46% 30% at 50% -6%,rgba(217,182,111,.1),transparent 70%),var(--bg);font-family:"Outfit",Inter,ui-sans-serif,-apple-system,'Segoe UI',sans-serif;font-size:16px;line-height:1.6;color:var(--text);padding:26px;-webkit-font-smoothing:antialiased}.wrap{width:min(1240px,100%);margin:0 auto}.top{height:58px;display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:26px}.brand{display:flex;align-items:center;gap:11px;text-decoration:none;color:inherit}.logo{width:34px;height:34px;display:grid;place-items:center;color:var(--gold)}.logo svg{width:26px;height:26px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}.brand strong,.brand span{display:block}.brand strong{font-family:var(--serif);font-weight:520;font-size:17px;letter-spacing:-.01em}.brand span{font-family:var(--mono);font-size:10px;letter-spacing:.22em;text-transform:uppercase;color:var(--muted2);margin-top:3px}.account{display:flex;align-items:center;gap:10px;color:var(--muted);font-size:12.5px}.ghost{min-height:40px;padding:0 15px;border-radius:11px;border:1px solid var(--line2);background:rgba(245,241,232,.03);color:var(--text);font-family:inherit;font-size:13px;font-weight:600;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;transition:.2s var(--ease)}.ghost:hover{background:rgba(245,241,232,.07);transform:translateY(-1px)}.hero{text-align:center;max-width:780px;margin:0 auto 20px}.eyebrow{display:inline-flex;align-items:center;gap:9px;color:var(--gold);font-family:var(--mono);font-size:11.5px;font-weight:500;letter-spacing:.22em;line-height:1.6;text-transform:uppercase}.hero h1{font-family:var(--serif);font-weight:400;font-size:clamp(34px,5vw,64px);line-height:.99;letter-spacing:-.028em;margin:14px 0 12px}.hero h1 em{display:block;font-style:italic;color:var(--gold2)}.hero p{color:var(--muted);font-size:14px;line-height:1.55;margin:0 auto;max-width:64ch}.wallet{padding:20px 22px;margin:0 0 28px;border:1px solid var(--line2);border-radius:20px;background:linear-gradient(160deg,var(--panel),var(--bg1) 70%);box-shadow:0 30px 90px rgba(0,0,0,.4)}
.wallet-top{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:16px}
.wallet-top strong{font-size:13.5px;font-weight:600}
.wallet-plan{min-height:24px;padding:0 10px;display:inline-flex;align-items:center;border-radius:8px;border:1px solid rgba(217,182,111,.32);background:rgba(217,182,111,.07);color:var(--gold2);font-family:var(--mono);font-size:10px;letter-spacing:.14em;text-transform:uppercase}
...
.wallet-fact{padding:14px 16px;background:var(--bg)}
.wallet-fact span{display:block;font-family:var(--mono);font-size:10px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted2)}
.wallet-fact b{display:block;margin-top:6px;font-family:var(--serif);font-size:27px;font-weight:480;letter-spacing:-.02em;line-height:1.05}
.wallet-fact i{display:block;margin-top:4px;font-size:11.5px;font-style:normal;color:var(--muted);line-height:1.45}
.bar{height:5px;background:rgba(245,241,232,.08);border-radius:999px;overflow:hidden;margin-top:16px}.bar i{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,var(--gold),var(--gold2));width:${pct}%}.section-title{text-align:center;max-width:720px;margin:56px auto 24px}.section-title span{font-family:var(--mono);font-size:10.5px;text-transform:uppercase;letter-spacing:.22em;color:var(--gold)}.section-title h2{font-family:var(--serif);font-weight:440;font-size:clamp(24px,2.6vw,34px);letter-spacing:-.018em;margin:10px 0 8px}.section-title p{color:var(--muted);font-size:14px;margin:0}.plans{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}.dc-plan,.dc-topup{border:1px solid var(--line2);border-radius:22px;background:var(--panel);padding:28px 26px;display:flex;flex-direction:column;gap:15px;position:relative}.dc-plan.featured,.dc-topup.featured{border-color:rgba(217,182,111,.5);background:linear-gradient(165deg,rgba(217,182,111,.06),var(--panel) 45%)}.plan-top{display:flex;justify-content:space-between;align-items:center}.badge,.popular{min-height:24px;padding:0 9px;display:inline-flex;align-items:center;border-radius:8px;background:rgba(217,182,111,.09);color:var(--gold2);font-family:var(--mono);font-size:10px;letter-spacing:.12em;text-transform:uppercase}.popular{background:linear-gradient(120deg,var(--gold),var(--gold2));color:#191204;font-weight:700}.dc-plan h2,.dc-topup h3{font-family:var(--serif);font-weight:460;font-size:24px;margin:0;letter-spacing:-.015em}.money{font-family:var(--serif);font-weight:480;font-size:33px;letter-spacing:-.02em;color:var(--gold2)}.money small,.topup-price small{font-family:"Outfit",sans-serif;font-size:12.5px;color:var(--muted2);font-weight:500;letter-spacing:0}.dc-plan p,.dc-topup p{color:var(--muted);font-size:12.5px;line-height:1.55;margin:0}.dc-plan>p{min-height:40px}.tokens{padding:15px 16px;border-radius:13px;background:rgba(245,241,232,.03);border:1px solid var(--line)}.tokens b,.tokens span{display:block}.tokens b{font-family:var(--serif);font-weight:480;font-size:23px}.tokens span{font-size:10.5px;color:var(--muted2);margin-top:2px}.dc-plan ul{margin:0;padding:0;list-style:none;display:grid;gap:11px;color:var(--muted);font-size:12.5px;line-height:1.5}.dc-plan li:before{content:'✓';color:var(--green);margin-right:8px;font-size:11px}.dc-plan button,.dc-topup button,@keyframes dcRise{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:none}}@keyframes dcPrice{from{opacity:0;transform:translateY(-7px)}to{opacity:1;transform:none}}@keyframes dcFade{from{opacity:0}to{opacity:1}}.hero,.wallet,.section-title,.period-switch,.dc-topup{animation:dcRise .55s var(--ease) both}.dc-plan,.dc-topup{transition:transform .25s var(--ease),border-color .25s ease,box-shadow .25s ease}.topups .dc-topup:nth-child(2){animation-delay:.08s}.topups .dc-topup:nth-child(3){animation-delay:.16s}.dc-plan:hover{transform:translateY(-5px);border-color:rgba(217,182,111,.45);box-shadow:0 32px 66px rgba(0,0,0,.45)}.dc-topup:hover{transform:translateY(-4px);border-color:rgba(217,182,111,.36)}@keyframes dcSlideA{from{opacity:0;transform:translateX(34px)}to{opacity:1;transform:none}}@keyframes dcSlideB{from{opacity:0;transform:translateX(34px)}to{opacity:1;transform:none}}@keyframes dcSlideC{from{opacity:0;transform:translateX(34px)}to{opacity:1;transform:none}}#per-weekly:checked~.plans .dc-plan{animation:dcSlideA .38s var(--ease) both}#per-monthly:checked~.plans .dc-plan{animation:dcSlideB .38s var(--ease) both}#per-yearly:checked~.plans .dc-plan{animation:dcSlideC .38s var(--ease) both}.plans .dc-plan:nth-child(2){animation-delay:.05s!important}.plans .dc-plan:nth-child(3){animation-delay:.1s!important}.dc-plan button,.dc-topup button{transition:transform .16s ease,filter .16s ease}.dc-plan button:not(:disabled):hover,.dc-topup button:not(:disabled):hover{transform:translateY(-2px);filter:brightness(1.07)}@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation:none!important;transition:none!important}}.dcperiod{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.period-switch{position:relative;display:flex;align-items:center;justify-content:center;gap:4px;width:max-content;max-width:100%;margin:0 auto 26px;padding:4px;border:1px solid var(--line2);border-radius:13px;background:var(--panel)}.period-switch label{position:relative;z-index:1;width:104px;text-align:center;padding:10px 0;border-radius:10px;font-size:13px;font-weight:600;color:var(--muted);cursor:pointer;transition:color .18s ease}.period-switch label:hover{color:var(--text)}.period-switch::before{content:'';position:absolute;top:4px;left:4px;width:104px;height:calc(100% - 8px);border-radius:10px;background:linear-gradient(120deg,var(--gold),var(--gold2));transition:transform .32s var(--ease)}.period-note{display:none;text-align:center;margin:-8px auto 16px;font-size:12px;font-weight:600;color:var(--gold2)}#per-weekly:checked~.period-switch label[for=per-weekly],#per-monthly:checked~.period-switch label[for=per-monthly],#per-yearly:checked~.period-switch label[for=per-yearly]{color:#191204}#per-weekly:checked~.period-switch::before{transform:translateX(0)}#per-monthly:checked~.period-switch::before{transform:translateX(108px)}#per-yearly:checked~.period-switch::before{transform:translateX(216px)}#per-yearly:checked~.period-note{display:block}.dcperiod:focus-visible~.period-switch{outline:2px solid var(--gold2);outline-offset:3px}.plans .per-weekly,.plans .per-monthly,.plans .per-yearly{display:none}#per-weekly:checked~.plans .per-weekly,#per-monthly:checked~.plans .per-monthly,#per-yearly:checked~.plans .per-yearly{display:block}.dc-plan form{margin-top:auto}.dc-plan button:disabled,.dc-topup button:disabled{opacity:.42;cursor:not-allowed}.topups{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:20px}.topup-value b,.topup-value span{display:block}.topup-value b{font-family:var(--serif);font-weight:480;font-size:37px;letter-spacing:-.02em;color:var(--gold2)}.topup-value span{font-size:11px;color:var(--muted2)}.topup-price{font-size:19px;font-weight:650}.shop-note{text-align:center;color:var(--muted2);font-size:12px;margin:16px auto 0;max-width:760px}.hero-go{display:flex;flex-direction:column;align-items:center;gap:11px;margin-top:19px;animation:dcFade .5s ease both}.hero-go button{min-height:46px;padding:0 26px;border:1px solid rgba(217,182,111,.5);border-radius:12px;background:rgba(217,182,111,.09);color:var(--gold2);font-family:inherit;font-size:14px;font-weight:650;cursor:pointer;transition:transform .16s ease,background .16s ease}.hero-go button:hover{transform:translateY(-1px);background:rgba(217,182,111,.15)}.hero-go>span{color:var(--muted);font-size:12.5px;line-height:1.55;max-width:62ch}@media(max-width:900px){}.alerts{margin-bottom:14px}.alert{padding:12px 14px;border-radius:13px;font-size:12.5px;margin-bottom:8px}.alert.bad{background:rgba(239,107,122,.1);border:1px solid rgba(239,107,122,.25);color:#ffb7bf}.alert.good{background:rgba(111,206,158,.1);border:1px solid rgba(111,206,158,.25);color:#baffd5}.foot{text-align:center;color:var(--muted2);font-size:11.5px;line-height:1.65;margin:28px auto 8px;max-width:850px}@media(max-width:900px){body{padding:16px}.plans,.topups,.wallet-facts{grid-template-columns:1fr}.hero h1{font-size:40px}.top{height:auto;align-items:flex-start;flex-direction:column;margin-bottom:30px}}@media(max-width:560px){.top{height:auto;padding:2px 0;margin-bottom:16px}.account>span{display:none}.ghost{min-height:36px;padding:0 12px;font-size:12px}.account span{display:none}.hero h1{font-size:34px}.dc-plan,.dc-topup{border-radius:18px}}
  /* The code row. Quiet until a grant is running, then gold -- it is the one
     thing on the page a tester wants to read, and spending the colour on the
     empty state would make it look like every other box. */
  .code-row{display:flex;align-items:center;justify-content:space-between;gap:22px;flex-wrap:wrap;margin:0 0 18px;padding:18px 22px;border:1px solid var(--line);border-radius:16px;background:var(--bg1)}
  .code-row[data-on]{border-color:rgba(217,182,111,.34);background:rgba(217,182,111,.055)}
  .code-say{flex:1 1 300px;min-width:0}
  .code-say strong{display:block;font-size:15.5px;font-weight:600;color:var(--text)}
  .code-say span{display:block;margin-top:4px;font-size:12.5px;line-height:1.55;color:var(--muted)}
  .code-row form{display:flex;gap:9px;flex:0 1 340px;min-width:0}
  .code-row input{flex:1 1 auto;min-width:0;height:44px;padding:0 13px;border-radius:10px;border:1px solid var(--line2);background:var(--bg);color:var(--text);font:600 14px/1 ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.08em;text-transform:uppercase}
  .code-row input:focus{outline:none;border-color:rgba(217,182,111,.55)}
  .code-row button{flex:none;height:44px;padding:0 18px;border:0;border-radius:10px;cursor:pointer;background:linear-gradient(120deg,var(--gold),var(--gold2));color:#171109;font:700 13.5px/1 inherit}
  /* The free window, once it has closed. A statement, not an offer. */
  .free-done{margin:0 0 18px;padding:17px 22px;border:1px solid var(--line);border-radius:16px;background:var(--bg1)}
  .free-done strong{display:block;font-size:15.5px;font-weight:600;color:var(--text)}
  .free-done span{display:block;margin-top:5px;font-size:12.5px;line-height:1.55;color:var(--muted)}
  @media(max-width:640px){.code-row{padding:16px 17px}.code-row form{flex:1 1 100%}}
  </style></head><body><main class="wrap"><div class="top"><a class="brand" href="/"><div class="logo" aria-hidden="true"><svg viewBox="0 0 32 32"><path d="M16 3.5c-5.8 0-10.5 4.7-10.5 10.5v14.5h21V14c0-5.8-4.7-10.5-10.5-10.5Z"/></svg></div><div><strong>DeenClipped</strong><span>Plans & token shop</span></div></a><div class="account"><span>${esc(user?.email || user?.name || 'Signed in')}</span><a class="ghost" href="/app">Dashboard</a><form method="post" action="/auth/logout"><button class="ghost" type="submit">Log out</button></form></div></div><div class="alerts">${error ? `<div class="alert bad">${esc(error)}</div>` : ''}${info ? `<div class="alert good">${esc(info)}</div>` : ''}</div><section class="hero"><span class="eyebrow">Simple, affordable creator pricing</span><h1>Choose a plan. <em>Add tokens any time.</em></h1><p>Pricing is tied to the source minutes you select. A subscription refreshes every period; one-time top-up tokens stay in your wallet until you spend them.</p>${heroGo}</section>${codeBox}${freeDone}<input type="radio" name="dcperiod" id="per-weekly" class="dcperiod"><input type="radio" name="dcperiod" id="per-monthly" class="dcperiod" checked><input type="radio" name="dcperiod" id="per-yearly" class="dcperiod"><div class="period-switch"><label for="per-weekly">Weekly</label><label for="per-monthly">Monthly</label><label for="per-yearly">Yearly</label></div><p class="period-note">Two months free on every yearly plan</p><section class="plans">${planCards}</section><section class="wallet"><div class="wallet-top"><strong>Your wallet</strong><span class="wallet-plan">${esc(cur.plan || 'free')} plan</span></div><div class="wallet-facts"><div class="wallet-fact"><span>Available now</span><b>${esc(remaining)}</b><i>tokens you can spend today</i></div><div class="wallet-fact"><span>Used this period</span><b>${cur.unlimited ? '\u2014' : esc(String(Math.round(Number(cur.used || 0))))}</b><i>${cur.unlimited ? 'no usage limit on this plan' : 'resets when the plan renews'}</i></div><div class="wallet-fact"><span>Top-ups</span><b>${cur.unlimited ? '\u221e' : esc(String(Math.round(Number(cur.bonusTokens || 0))))}</b><i>bought tokens, never expire</i></div><div class="wallet-fact"><span>What it costs</span><b>${esc(String(bill.tokenRatePerMinute || 1))}/min</b><i>only the range you select is charged</i></div></div><div class="bar"><i></i></div></section><div class="section-title"><span>Token shop</span><h2>Need more without changing your plan?</h2><p>Buy a one-time token pack. Top-up tokens are added to your current wallet and do not disappear at your next renewal.</p></div><section class="topups" id="token-shop">${topupCards}</section><p class="shop-note">Token packs are optional and available alongside free, weekly, monthly and yearly plans. Stripe shows the final total before payment.</p><div class="foot">Payments are handled by Stripe. DeenClipped does not store complete card details. Prices shown are configuration-driven labels and Stripe shows the final amount before payment.</div></main></body></html>`;
}
