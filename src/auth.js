...
/*
 * APPLE'S CLIENT ID IS A SERVICES ID, AND ITS SHAPE IS PART OF BEING
 * CONFIGURED.
 *
 * Found live on production, 7 Sept 2026: all four APPLE_SIGNIN_* variables
 * were set, so `configured('apple')` was true, so the sign-in page drew a live
 * "Continue with Apple" button -- and pressing it took a real customer to
 * Apple's own error page reading "invalid_client -- Invalid client." The value
 * in APPLE_SIGNIN_CLIENT_ID was a 32-character hex string, which is not a
 * Services ID at all; it looks like a placeholder pasted in to fill the field,
 * and filling the field is exactly what switched the button on.
 *
 * A Services ID is a reverse-DNS identifier (`online.deenclipped.signin`).
 * Apple never issues a bare hex one, so a client id with no dot in it cannot
 * be accepted and is not configuration -- it is a blank with something typed
 * in it. Checking the SHAPE is what lets the button turn itself back on the
 * day real credentials are set, with nobody having to remember.
 *
 * This does NOT claim to validate the credential: only Apple can say whether a
 * well-formed Services ID is live. It catches the shape that was actually
 * shipping a broken button, and says so rather than pretending to more.
 */
function looksLikeAppleServiceId(value) {
...
function googleButton(returnTo) {
  return `<a class="dc-oauth-btn" href="/auth/google/start?returnTo=${encodeURIComponent(safeReturn(returnTo))}"><svg viewBox="0 0 24 24" aria-hidden="true"><path fill="#4285F4" d="M23 12.3c0-.8-.1-1.6-.2-2.3H12v4.5h6.2a5.3 5.3 0 0 1-2.3 3.5v2.9h3.7c2.2-2 3.4-5 3.4-8.6Z"/><path fill="#34A853" d="M12 24c3.1 0 5.7-1 7.6-2.8l-3.7-2.9c-1 .7-2.3 1.1-3.9 1.1-3 0-5.5-2-6.4-4.7H1.8v3A12 12 0 0 0 12 24Z"/><path fill="#FBBC05" d="M5.6 14.7a7.2 7.2 0 0 1 0-4.6v-3H1.8a12 12 0 0 0 0 10.6l3.8-3Z"/><path fill="#EA4335" d="M12 4.8c1.7 0 3.2.6 4.4 1.7l3.3-3.3A11.6 11.6 0 0 0 1.8 7.1l3.8 3c.9-2.8 3.4-4.7 6.4-4.7Z"/></svg>Continue with Google</a>`;
}

function appleButton(returnTo) {
  return `<a class="dc-oauth-btn apple" href="/auth/apple/start?returnTo=${encodeURIComponent(safeReturn(returnTo))}"><svg viewBox="0 0 24 24" aria-hidden="true" fill="currentColor" stroke="none"><path d="M16.4 12.7c0-2.3 1.9-3.4 2-3.5-1.1-1.6-2.8-1.8-3.4-1.8-1.4-.1-2.8.9-3.5.9s-1.8-.9-3-.8c-1.5 0-2.9.9-3.7 2.3-1.6 2.7-.4 6.7 1.1 8.9.8 1.1 1.7 2.3 2.9 2.2 1.2 0 1.6-.7 3-.7s1.8.7 3 .7 2-1.1 2.8-2.2c.9-1.2 1.2-2.4 1.2-2.5 0 0-2.4-.9-2.4-3.5ZM14.2 5.9c.6-.8 1-1.9.9-3-.9 0-2 .6-2.7 1.4-.6.7-1.1 1.8-.9 2.9 1 0 2.1-.5 2.7-1.3Z"/></svg>Continue with Apple</a>`;
}
...
export function loginPage({ error = '', returnTo = '/', info = '' } = {}) {
  ...
  /*
   * A PROVIDER THAT CANNOT WORK IS NOT DRAWN AT ALL.
   *
   * These used to render greyed (`pointer-events:none; opacity:.45`) when the
   * credentials were missing. Inert, but still a dead control on the first
   * screen a customer ever sees -- and it read as a fault rather than as an
   * option that is not on offer here. Invariant 9 applies to the sign-in page
   * as much as to the studio.
   *
   * The row and the "or use email" rule go with them: a divider under nothing
   * is a line across the top of a form, and the word "or" promises an
   * alternative that is not there.
   */
  const oauthButtons = [
    providers.google ? googleButton(returnTo) : '',
    providers.apple ? appleButton(returnTo) : '',
  ].filter(Boolean).join('');
  const oauthBlock = oauthButtons
    ? `<div class="dc-auth-oauth">${oauthButtons}</div><div class="dc-auth-divider">or use email</div>`
    : '';
  ...
  *{box-sizing:border-box}html{min-width:320px}
  ...
  a{color:inherit}
  .dc-auth-nav{position:relative;z-index:3;height:68px;border-bottom:1px solid rgba(245,241,232,.06);background:rgba(7,6,7,.85);backdrop-filter:blur(14px)}
  .dc-auth-nav-inner{width:min(calc(100% - 48px),1280px);height:100%;margin:auto;display:flex;align-items:center;justify-content:space-between;gap:24px}
  .dc-brand{display:inline-flex;align-items:center;gap:11px;text-decoration:none}
  .dc-brand-mark{width:30px;height:30px;display:grid;place-items:center;color:var(--gold)}
  .dc-brand-mark svg{width:24px;height:24px;stroke:currentColor;stroke-width:2}
  .dc-brand-copy strong,.dc-brand-copy small{display:block;white-space:nowrap}
  .dc-brand-copy strong{font-family:var(--serif);font-weight:520;font-size:17px;letter-spacing:-.01em}
  .dc-brand-copy small{margin-top:3px;color:var(--muted2);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;letter-spacing:.28em;text-transform:uppercase}
  .dc-back-link{min-height:42px;display:inline-flex;align-items:center;gap:8px;padding:0 15px;border:1px solid var(--line2);border-radius:11px;background:rgba(245,241,232,.04);color:var(--muted);text-decoration:none;font-size:13px;font-weight:600;transition:.2s var(--ease)}
  .dc-back-link:hover{background:rgba(245,241,232,.08);color:#fff;transform:translateY(-1px)}
  .dc-back-link svg{width:15px;height:15px;fill:none;stroke:currentColor;stroke-width:1.8}
  .dc-auth-shell{position:relative;z-index:1;width:min(calc(100% - 48px),1280px);min-height:calc(100vh - 68px);margin:auto;padding:34px 0 42px;display:grid;grid-template-columns:minmax(0,1.14fr) minmax(400px,.86fr);gap:58px;align-items:center}
  .dc-auth-story{min-width:0}
  .dc-eyebrow{display:inline-flex;align-items:center;gap:9px;color:var(--gold);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:11px;font-weight:500;letter-spacing:.22em;text-transform:uppercase}
  .dc-eyebrow i{width:22px;height:1px;background:var(--gold);opacity:.7}
  .dc-auth-story h1{max-width:760px;margin:18px 0 15px;font-family:var(--serif);font-size:clamp(42px,4.8vw,64px);line-height:1.01;letter-spacing:-.026em;font-weight:420}
  .dc-auth-story h1 span{font-style:italic;font-weight:380;color:var(--gold2)}
  .dc-story-copy{max-width:640px;margin:0;color:var(--muted);font-size:16px;line-height:1.62}
  .dc-capabilities{display:flex;align-items:center;gap:11px;flex-wrap:wrap;margin-top:20px;color:var(--muted)}
  .dc-capabilities span{display:inline-flex;align-items:center;gap:8px;font-size:12.5px;white-space:nowrap}
  .dc-capabilities b{width:21px;height:21px;display:grid;place-items:center;border-radius:7px;background:rgba(217,182,111,.1);color:var(--gold2);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px}
  .dc-capabilities i{width:3px;height:3px;border-radius:50%;background:#4e4b47}
  .dc-product-stage{position:relative;max-width:760px;height:280px;margin-top:30px;border:1px solid var(--line2);border-radius:20px;background:var(--bg2);box-shadow:var(--shadow)}
  .dc-product-stage>img{display:block;width:100%;height:100%;object-fit:cover;object-position:top center;border-radius:19px;background:var(--bg)}
  .dc-reel{position:absolute;z-index:3;width:82px;border:1px solid rgba(245,241,232,.16);border-radius:14px;overflow:hidden;background:var(--bg2);box-shadow:var(--shadow)}
  .dc-reel img{display:block;width:100%;aspect-ratio:9/16;object-fit:cover}
  .dc-reel.one{left:-27px;bottom:-20px;transform:rotate(-5deg)}
  .dc-reel.two{right:-25px;top:-17px;transform:rotate(5deg)}
  .dc-status{position:absolute;z-index:4;display:flex;align-items:center;gap:9px;padding:9px 12px;border:1px solid rgba(245,241,232,.14);border-radius:12px;background:rgba(10,9,11,.92);backdrop-filter:blur(14px);box-shadow:0 18px 48px rgba(0,0,0,.5)}
  .dc-status strong,.dc-status small{display:block}
  .dc-status strong{font-size:11.5px;font-weight:600}
  .dc-status small{margin-top:2px;color:var(--muted2);font-size:10.5px}
  .dc-status-icon{width:28px;height:28px;display:grid;place-items:center;border-radius:9px;background:rgba(217,182,111,.12);color:var(--gold2);font-size:11px}
  .dc-status.found{left:62px;top:-13px}
  .dc-status.ready{right:45px;bottom:-16px}
  .dc-auth-card{width:100%;max-width:470px;justify-self:end;padding:32px;border:1px solid var(--line2);border-radius:22px;background:linear-gradient(160deg,var(--bg2),var(--bg1) 70%);box-shadow:var(--shadow)}
  .dc-auth-card h2{margin:0;font-family:var(--serif);font-size:31px;line-height:1.04;letter-spacing:-.02em;font-weight:460}
  .dc-auth-card>p{margin:9px 0 24px;color:var(--muted);font-size:14px;line-height:1.55}
  .dc-alert{padding:11px 13px;border-radius:12px;margin-bottom:13px;font-size:12.5px;line-height:1.45}
  .dc-alert.bad{background:rgba(239,107,122,.1);border:1px solid rgba(239,107,122,.24);color:#ffb7bf}
  .dc-alert.good{background:rgba(111,206,158,.1);border:1px solid rgba(111,206,158,.24);color:#c5ffdd}
  .dc-auth-form{display:grid;gap:8px}
  .dc-auth-form label{font-size:12.5px;font-weight:600;color:var(--muted)}
  .dc-auth-form input{width:100%;box-sizing:border-box;height:48px;border:1px solid var(--line2);outline:0;background:var(--bg);color:var(--text);border-radius:12px;padding:0 14px;font-family:var(--sans);font-size:15px;transition:border-color .2s var(--ease),box-shadow .2s var(--ease)}
  .dc-auth-form input::placeholder{color:var(--muted2)}
  .dc-auth-form input:focus{border-color:rgba(217,182,111,.55);box-shadow:0 0 0 3px rgba(217,182,111,.09)}
  .dc-email-form{padding:0;border:0;background:none}
  .dc-auth-primary,.dc-oauth-btn{width:100%;height:50px;border-radius:12px;font:inherit;font-size:14.5px;font-weight:600;cursor:pointer}
  .dc-auth-primary{margin-top:2px;border:0;font-weight:700;background:linear-gradient(120deg,var(--gold),var(--gold2));color:#191204;transition:filter .2s}
  .dc-auth-primary:hover{filter:brightness(1.06)}
  .dc-auth-primary.compact{height:43px}
  .dc-email-hint{color:var(--muted2);font-size:11.5px;line-height:1.5}
  .dc-auth-divider{display:flex;align-items:center;gap:12px;color:var(--muted2);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10.5px;letter-spacing:.14em;text-transform:uppercase;margin:22px 0}
  .dc-auth-divider:before,.dc-auth-divider:after{content:'';height:1px;background:var(--line);flex:1}
  .dc-auth-oauth{display:flex;flex-direction:column;gap:10px}
  .dc-oauth-btn{display:flex;align-items:center;justify-content:center;gap:10px;text-decoration:none;border:1px solid transparent;background:#f5f1e8;color:#191204;transition:.2s var(--ease)}
  .dc-oauth-btn:hover{background:#fff;transform:translateY(-1px)}
  .dc-oauth-btn.apple{background:rgba(245,241,232,.04);color:var(--text);border-color:var(--line2)}
  .dc-oauth-btn.apple:hover{background:rgba(245,241,232,.08)}
  .dc-oauth-btn svg{width:19px;height:19px;flex:none}
  .dc-oauth-btn:disabled{opacity:.36;cursor:not-allowed;filter:saturate(.25)}
  .dc-admin-login{margin-top:12px;border:1px solid var(--line);border-radius:12px;background:rgba(245,241,232,.02);padding:0 13px}
  .dc-admin-login summary{min-height:44px;display:flex;align-items:center;cursor:pointer;color:var(--muted);font-size:12.5px}
  .dc-admin-login .dc-auth-form{padding:0 0 13px}
  .dc-legal-consent{margin:14px 0 0;color:var(--muted);font-size:11.5px;line-height:1.55}
  .dc-legal-consent a{color:var(--gold2);font-weight:600;text-decoration:none}
  .dc-legal-consent a:hover{text-decoration:underline}
  .dc-foot{display:flex;align-items:flex-start;gap:8px;margin-top:20px;padding-top:18px;border-top:1px solid var(--line);color:var(--muted2);font-size:11.5px;line-height:1.5}
  .dc-foot svg{width:12px;height:12px;flex:none;margin-top:1px;fill:none;stroke:var(--green);stroke-width:2}
  @media(max-width:1040px){.dc-auth-shell{grid-template-columns:minmax(0,1fr) minmax(380px,.84fr);gap:35px}.dc-auth-story h1{font-size:48px}.dc-product-stage{height:245px}.dc-status.found{left:36px}.dc-status.ready{right:28px}}
...
export function resetPage({ token: resetToken = '', error = '', info = '', done = false } = {}) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Reset password · DeenClipped</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Fraunces:opsz,wght@9..144,300..700&family=Outfit:wght@400;500;600;700&display=swap"><style>
  :root{color-scheme:dark;--bg:#070607;--bg1:#0d0c0e;--bg2:#131115;--text:#f5f1e8;--muted:#a8a196;--muted2:#736d63;--line:rgba(245,241,232,.12);--gold:#d9b66f;--gold2:#f1d18e}
  *{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:24px;background:radial-gradient(46% 34% at 30% 0%,rgba(217,182,111,.09),transparent 70%),var(--bg);color:var(--text);font-family:"Outfit",Inter,ui-sans-serif,-apple-system,'Segoe UI',sans-serif}
  .card{width:100%;max-width:430px;padding:32px;border:1px solid rgba(245,241,232,.17);border-radius:22px;background:linear-gradient(160deg,var(--bg2),var(--bg1) 70%);box-shadow:0 40px 120px rgba(0,0,0,.55)}
  h2{margin:0 0 8px;font-family:"Fraunces",Georgia,serif;font-weight:460;font-size:28px;letter-spacing:-.02em}
  ...
  input{height:48px;padding:0 14px;border:1px solid var(--line);border-radius:12px;background:var(--bg);color:var(--text);font-family:inherit;font-size:15px;outline:0}
  input:focus{border-color:rgba(217,182,111,.55);box-shadow:0 0 0 3px rgba(217,182,111,.09)}
  .btn{display:block;width:100%;height:50px;margin-top:6px;border:0;border-radius:12px;background:linear-gradient(120deg,var(--gold),var(--gold2));color:#191204;font:inherit;font-size:14.5px;font-weight:700;text-align:center;line-height:50px;text-decoration:none;cursor:pointer;transition:filter .2s}
  ...
  .alert.good{background:rgba(111,206,158,.1);border:1px solid rgba(111,206,158,.24);color:#c5ffdd}
  .back{display:block;margin-top:18px;color:var(--muted2);font-size:12px;text-decoration:none}
