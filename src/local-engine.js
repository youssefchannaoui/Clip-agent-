...
export async function submitVideo(url, title = '', ownerId = '', options = {}) {
  ...
  const job = {
    ...
    settings: sharedSettings(), sourceStartSec: sourceRange.startSec || 0, sourceEndSec: sourceRange.endSec || null,
    sourceTitle: sourceMeta?.title || null, sourceDurationSec: sourceMeta?.durationSec || null, sourceThumbUrl: sourceMeta?.thumbnail || null,
  };
  ...
  log(`Queued "${project.title}" for the self-hosted clip AI using template "${template.name}"${rangeCopy}.`);
