...
function importRerenderResult(jobRecord, file) {
  ...
  if (jobRecord.asVariant) {
    ...
    const preserved = {
      ...
      addedAt: original.addedAt, approvedAt: original.approvedAt, projectId: original.projectId,
...
export function queueMoreClips(projectId, requestedCount = 8) {
  ...
  // A template save can touch every scheduled/unposted clip at once. Replace
  // older queued renders for the same clip so the newest template always wins
  // and the queue cannot fill with stale versions. A currently processing job
  // is allowed to finish, but importRerenderResult below refuses to overwrite a
  // newer queued result.
  for (const existing of state.rerenderJobs) {
    if (existing.clipId === clip.id && !existing.asVariant && existing.status === 'queued') {
      existing.status = 'superseded';
      existing.stage = 'Replaced by a newer template save';
