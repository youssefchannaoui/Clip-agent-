...
/*
 * The two pages that ship. `legacyPrivacy` and `legacyTerms` were left behind
 * by the 11 September rewrite -- renamed rather than deleted, referenced by
 * nothing, and still carrying the old disclosures word for word. Dead legal
 * copy in the same file as live legal copy is a trap: a probe written against
 * "the cookie section" spliced the wrong one on 14 Sept and reported a guard
 * as passing when it was not. Removed.
 */
export function privacy({ base, currentUser }) {
  /*
   * REWRITTEN 14 SEPT 2026, after the 11 September rewrite removed roughly
   * nine tenths of it -- including every disclosure the code still owed.
   *
   * What had gone: four cookies the site sets and none of them named, the
   * 30-day retention period the sweep in youtube-retention.js still enforces,
   * the Limited Use affirmation, the deletion route, and the yt-dlp/Webshare
   * import disclosure that had been made voluntarily and that a Google
   * reviewer may already have read.
   *
   * EVERY SENTENCE HERE IS TRUE OF THE DEPLOYED CODE, and that is the whole
   ...
   */
  const body = `<main><section class="page-hero wrap"><span class="eyebrow"><i></i>Legal</span><h1>Privacy Policy</h1><p>How DeenClipped handles account, source and publishing information.</p></section><section class="page-content"><article class="legal">`
    + `<p>Last updated: 14 September 2026</p>`
    + `<p>DeenClipped helps you create, review and publish short-form clips from long videos. This policy explains what is collected, why, how long it is kept and how to remove it.</p>`

    + `<h2>Account and workspace data</h2>`
    ...
    + `<p>You are responsible for submitting only videos you own or are authorised to use, and DeenClipped asks you to confirm that before a job starts. A public video URL or a file you upload is processed to create the clips you ask for. An import can fail because of source availability, rights, provider access or platform restrictions.</p>`

    + `<h2>YouTube URL processing</h2>`
    + `<p>When you paste a link, the download is performed by <strong>yt-dlp</strong> running on DeenClipped’s own processing server, and it may route through a <strong>Webshare</strong> residential proxy network so that requests come from a stable address. <strong>No Google credentials are sent</strong> to the downloader or to the proxy network, and no Google API is used to fetch, search or list anyone’s videos. Only the part of the video you select is downloaded.</p>`
    ...
    + `<h2>Ask DeenAI</h2>`
    + `<p>Transcribing, scoring and titling your lectures all happen on DeenClipped’s own processing server and are never sent to a third-party model. <strong>Ask DeenAI</strong> is the exception and it is opt-in: when you ask it a question, that question, figures counted from your own clips, and anything you deliberately attach to the conversation are sent to the AI provider that answers it. Your transcripts are not sent unless you attach them.</p>`

    + `<h2>Cookies and website analytics</h2>`
    + `<p>DeenClipped sets a small number of first-party cookies and <strong>no advertising cookies</strong>. There is no third-party advertising or tracking network on this site.</p>`
    + `<ul>`
    + `<li>A session cookie that keeps you signed in. It is removed when you sign out.</li>`
    + `<li><strong>dc_seen</strong> — a single digit recording that this browser has visited before, so new and returning visits can be counted. It contains no identifier and is kept for two years.</li>`
    + `<li><strong>dc_land</strong> — the path of the page you first arrived on, so we can tell which pages bring people to the product. It contains no identifier and is kept for 90 days.</li>`
    + `<li><strong>dc_ref</strong> — the invite code from a referral link, so the person who invited you is credited. It is kept for 60 days.</li>`
    + `</ul>`
    + `<p>Page visits are counted first-party. A visitor is identified by an irreversible hash that is re-salted every day, so the same browser cannot be recognised across two days, and no IP address, user agent or per-visit record is stored. Signed-in operators are excluded from the counts entirely.</p>`

    + `<h2>Security and storage</h2>`
    + `<p>Credentials are encrypted at rest. Where a platform issues them, DeenClipped stores <strong>encrypted OAuth access and refresh tokens</strong> scoped to the one account they belong to, and uses them only to publish clips you approve or schedule. Access is limited to operating the service.</p>`

    + `<h2>Retention and deletion</h2>`
    + `<p>You can remove everything yourself: <strong>Account settings</strong> → <strong>Delete my account</strong>. This deletes your account, lectures, clips, templates, nasheeds, schedules, billing history, sessions and stored platform credentials <strong>immediately and cannot be undone</strong>. Posts already published stay on the third-party platform and remain under your control there.</p>`
    + `<p>If you cannot sign in, email <a href="mailto:support@deenclipped.online?subject=Data%20deletion%20request">support@deenclipped.online</a> with the subject “Data deletion request” and it will be <strong>completed within 30 days</strong>. Deleting your account does not cancel a subscription; cancel that in the billing portal first.</p>`

    + `<h2>Contact</h2>`
    + `<p>For any privacy or data question, email <a href="mailto:support@deenclipped.online">support@deenclipped.online</a>.</p>`
    + `</article></section></main>`;
  return layout({ base, currentUser, ...meta('/privacy'), body });
}

export function terms({ base, currentUser }) {
  const body = `<main><section class="page-hero wrap"><span class="eyebrow"><i></i>Legal</span><h1>Terms of Service</h1><p>The rules for creating, reviewing and publishing clips with DeenClipped.</p></section><section class="page-content"><article class="legal">`
    + `<p>Last updated: 14 September 2026</p>`
    + `<p>These Terms govern use of DeenClipped, a service for creating, reviewing and publishing short-form clips from long videos.</p>`

    + `<h2>Your source content</h2>`
    ...
    + `<h2>Contact</h2>`
    + `<p>Questions can be sent to <a href="mailto:support@deenclipped.online">support@deenclipped.online</a>.</p>`
    + `</article></section></main>`;
