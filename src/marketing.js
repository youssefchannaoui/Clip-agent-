...
function pricingCards(currentUser = null) {
  ...
  return `<div class="pricing-grid">${plans.map(plan => `<article class="price-card ${plan.popular ? 'popular' : ''}">${plan.popular ? '<span class="popular-label">Most popular</span>' : ''}${plan.saving > 0 ? `<span class="popular-label saving">Save ${plan.saving}%</span>` : ''}<span class="plan-kicker">${escapeHtml(plan.kicker)}</span><h3>${escapeHtml(plan.name)}</h3><div class="plan-price-label">${plan.listPrice ? `<s>${escapeHtml(plan.listPrice)}</s> ` : ''}${escapeHtml(plan.price)}</div>${plan.perMonth ? `<div class="plan-permonth">works out to A$${escapeHtml(plan.perMonth)} a month</div>` : ''}<div class="price">${escapeHtml(plan.tokens)} <small>${escapeHtml(plan.interval)}</small></div><p>${escapeHtml(plan.copy)}</p>${perMinuteRate(plan.price, plan.tokens) ? `<div class="plan-permin">${escapeHtml(perMinuteRate(plan.price, plan.tokens))}</div>` : ''}<ul><li>Selected source-time processing</li><li>Review, editor and templates included</li><li>Ordinary template rerenders are free</li></ul>${plan.enabled ? `<a class="button ${plan.popular ? 'primary' : 'secondary'} full" href="${accountUrl}">${plan.id === 'free' ? 'Start free' : `Choose ${escapeHtml(plan.name.toLowerCase())}`}</a>` : '<span class="button secondary full disabled" aria-disabled="true">Stripe price not configured</span>'}</article>`).join('')}</div>`;
}
