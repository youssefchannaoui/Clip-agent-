...
function sourceForm() {
  return `<a class="source-bar" href="/login?returnTo=/app"><span class="source-icon">${icon('upload')}</span><span>Upload your original video to begin</span><b>Start clipping ${icon('arrow')}</b></a>`;
}
...
export function privacy({ base, currentUser }) {
  ...
  const compliantBody = body
    .replace(/<p>When you paste a link to a video for clipping,[\s\S]*?<\/p>/, '<p>DeenClipped does not use the YouTube API to access anyone else\'s videos or import audiovisual source files from public URLs.</p><h2>Source content</h2><p>DeenClipped accepts original video files uploaded by the creator. Before an upload starts, the creator must confirm that they own the source or have permission to repurpose it. DeenClipped does not import, download or retrieve audiovisual source files from public YouTube URLs.</p><h2>DeenAI processing</h2><p>Ask DeenAI runs on the private model hosted on the DeenClipped processing server and uses records already held in your workspace.</p>')
    .replace(/<p>Cached YouTube API Data[\s\S]*?<\/p>/, '<p>Cached YouTube API Data — the channel name and profile image of a connected channel — is automatically deleted after 30 days, in line with the YouTube API Services Developer Policies.</p>')
    .replace('<h2>Source content and retention</h2>', '<h2>Source content</h2><p>DeenClipped accepts original video files uploaded by the creator. Before an upload starts, the creator must confirm that they own the source or have permission to repurpose it. DeenClipped does not import, download or retrieve audiovisual source files from public YouTube URLs.</p><h2>Source content and retention</h2>');
  return layout({ base, currentUser, ...meta('/privacy'), body: compliantBody });
}
...
export function terms({ base, currentUser }) {
  ...
  const compliantBody = body
    .replace(/You are responsible for the videos, links, clips/, 'You are responsible for the videos, clips')
    .replace(/Uploading or importing videos you do not own or do not have permission to use may violate copyright or platform rules\. By using DeenClipped, you confirm that you have the required rights and permissions\./, 'Uploading videos you do not own or do not have permission to use may violate copyright or platform rules. DeenClipped accepts original source files only and requires a rights confirmation before upload.')
    .replace(/<p>Features may change,[\s\S]*?<\/p>/, '<p>Features may change, pause or be removed over time. Source-video processing is limited to original files uploaded by the creator. DeenClipped does not import, download or retrieve audiovisual source files from public YouTube URLs.</p>');
  return layout({ base, currentUser, ...meta('/terms'), body: compliantBody });
}
