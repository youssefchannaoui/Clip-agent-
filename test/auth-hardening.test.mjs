...
test('typing a listed operator address into the password form does not make an operator', () => {
  auth.ownerUser();
  const user = auth.emailLogin('operator@deenclipped.test', 'correct horse battery', 'Imposter');
  auth.ownerUser(); // runs elevateOperators
  assert.equal(state.authUsers.find(item => item.id === user.id).role, 'creator');
});

test('the same address proved by Google is elevated, and the unverified password stops working', () => {
  ...
  assert.throws(() => auth.emailLogin('operator@deenclipped.test', 'correct horse battery'), /Google or Apple/);
});

test('an unverified Google address is never linked to an existing account', () => {
  const victim = auth.emailLogin('victim@deenclipped.test', 'victims password 1');
...
test('a return path cannot be scheme-relative through a backslash', () => {
...
test('a malformed cookie does not turn every request into a 500', () => {
...
test('one account signing in many times cannot evict everyone else', () => {
  const alice = auth.emailLogin('alice@deenclipped.test', 'alices password 1');
  const bob = auth.emailLogin('bob@deenclipped.test', 'bobs password 12');
...
test('abandoned sign-in starts do not pile up in state', () => {
