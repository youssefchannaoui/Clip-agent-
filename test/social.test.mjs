...
test('YouTube OAuth, connection check and resumable upload complete', async () => {
  ...
  const start = social.oauthStartUrl('youtube');
  ...
  const checked = await social.testConnection('youtube');
  assert.equal(checked.accountId, 'channel-1');

  const target = { provider: 'youtube', accountId: 'channel-1', settings: { privacy: 'private', categoryId: '22' }, providerState: {} };
  const result = await social.publishTarget({ id: 'clip-1', title: 'Reminder', description: 'A useful reminder', hashtags: '#Islam' }, target, mediaFile);
...
test('Meta OAuth supports Instagram and Facebook publishing paths', async () => {
  ...
  const start = social.oauthStartUrl('meta');
  ...
  const checked = await social.testConnection('meta');
  assert.equal(checked.accounts[0].pageId, 'page-1');

  const igTarget = { provider: 'instagram', accountId: 'ig-1', settings: { shareToFeed: true }, providerState: {} };
  const pending = await social.publishTarget({ id: 'clip-ig', title: 'IG Reminder', description: 'Caption', hashtags: '#deen' }, igTarget, mediaFile);
  assert.equal(pending.pending, true);
  const igResult = await social.pollTarget({}, igTarget);
  ...
  const fbTarget = { provider: 'facebook', accountId: 'page-1', settings: {}, providerState: {} };
  const fbResult = await social.publishTarget({ id: 'clip-fb', title: 'FB Reminder', description: 'Caption', hashtags: '#deen', durationMs: 30_000 }, fbTarget, mediaFile);
...
test('TikTok OAuth, creator check, upload and publish polling complete', async () => {
  ...
  const start = social.oauthStartUrl('tiktok');
  ...
  const checked = await social.testConnection('tiktok');
  assert.equal(checked.accountId, 'tt-user');

  const target = { provider: 'tiktok', accountId: 'tt-user', settings: { privacy: 'SELF_ONLY', allowComments: true, allowDuet: false, allowStitch: false }, providerState: {} };
  const pending = await social.publishTarget({ id: 'clip-tt', title: 'TikTok Reminder', description: 'Caption', hashtags: '#deen', durationMs: 30_000 }, target, mediaFile);
  assert.equal(pending.pending, true);
  assert.equal(initBody.post_info.privacy_level, 'SELF_ONLY');
  assert.equal(initBody.post_info.disable_stitch, true);
  const result = await social.pollTarget({}, target);
