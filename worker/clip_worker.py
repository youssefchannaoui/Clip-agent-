    ...
    if mode == "dynamic-stack" and words:
        for frame in dynamic_caption_frames(candidate, template):
            ...
            for line_index, word in enumerate(frame["words"]):
                ...
                if is_active:
                    lines.append(f"{{\\c{override}\\fscx116\\fscy{int(scale_y * 1.16)}\\t(0,120,\\fscx100\\fscy{scale_y})}}{value}")
                else:
                    lines.append(f"{{\\c{override}}}{value}")
            text = "\\N".join(lines)
...
def build_video_filter(template: dict[str, Any], ass_file: Path, crop_plan: dict[str, Any] | None = None) -> str:
    ...
    if fit_mode == "crop":
        if crop_plan:
            ...
            pos_x = max(0.0, min(100.0, float(template.get("cropPositionX", 50.0)))) / 100.0
            pos_y = max(0.0, min(100.0, float(template.get("cropPositionY", 50.0)))) / 100.0
            scale_w = f"trunc(iw*max({width}/iw\\,{height}/ih)*{zoom:.4f}/2)*2"
            scale_h = f"trunc(ih*max({width}/iw\\,{height}/ih)*{zoom:.4f}/2)*2"
            graph = (
                f"[0:v]scale=w='{scale_w}':h='{scale_h}',"
                f"crop={width}:{height}:x='(in_w-out_w)*{pos_x:.6f}':y='(in_h-out_h)*{pos_y:.6f}',setsar=1[base]"
            )
