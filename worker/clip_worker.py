...
def write_ass(candidate: Candidate, template: dict[str, Any], ass_file: Path) -> None:
    ...
    custom_caption_x = template.get("captionPositionX")
    custom_caption_y = template.get("captionPositionY")
    position_override = ""
    if custom_caption_x is not None and custom_caption_y is not None:
        px = int(round(width * max(0.07, min(0.93, float(custom_caption_x) / 100.0))))
        py = int(round(height * max(0.08, min(0.88, float(custom_caption_y) / 100.0))))
        position_override = f"{{\\pos({px},{py})}}"
...
def track_speaker_keyframes(
    ...
    * **Speech activity** — `speech_spans` carries the Whisper word timings.
    ...
    previous_gray = None
    previous_subject: tuple[float, float, float] | None = None  # cx, cy, face area ratio

    def dedupe_faces(items: list[tuple[int, int, int, int]]) -> list[tuple[int, int, int, int]]:
        """Merge the same face returned by several Haar cascades.

        Without this, one face can appear two or three times and accidentally
        beat the real active speaker simply because multiple detectors found it.
        """
        ...
        for face in sorted(items, key=lambda f: f[2] * f[3], reverse=True):
            ...
            for kx, ky, kw, kh in kept:
                ix0, iy0 = max(fx, kx), max(fy, ky)
                ix1, iy1 = min(fx + fw, kx + kw), min(fy + fh, ky + kh)
    ...
    for index in range(samples + 1):
        ...
        if faces:
            ...
            for (fx, fy, fw, fh) in faces:
                ...
                area_ratio = float(fw * fh) / float(src_w * src_h)
                # Bigger faces are more likely to be the subject.
                score = area_ratio
                # Prefer continuity unless another face has convincing mouth
                # movement. This prevents one-frame jumps that cut the speaker
                # in half when several people are visible.
                if previous_subject is not None:
                    px, py, _ = previous_subject
                    distance = ((cx - px) ** 2 + (cy - py) ** 2) ** 0.5 / max(src_w, src_h)
                    score += max(0.0, 0.20 - distance) * 1.8
                # Reward crops that leave useful space around the face rather
                # than pinning it to the narrow portrait edge.
                proposed_x, _ = crop_origin_from_center(cx, cy, src_w, src_h, crop_w, crop_h, max(padding, 0.22))
                face_left = (fx - proposed_x) / max(1, crop_w)
                face_right = (fx + fw - proposed_x) / max(1, crop_w)
                if 0.10 <= face_left and face_right <= 0.90:
                    score += 0.08
            ...
            if best:
                fx, fy, fw, fh = best
                cx, cy = fx + fw / 2.0, fy + fh / 2.0
                previous_subject = (cx, cy, float(fw * fh) / float(src_w * src_h))
                raw.append((offset, cx, cy))
        elif previous_subject is not None:
            # Brief detector misses should hold the current speaker instead of
            # making the crop interpolate toward a different person.
    ...
    for (t, cx, cy) in raw:
        ...
        x, y = crop_origin_from_center(smooth_x, smooth_y, src_w, src_h, crop_w, crop_h, max(padding, 0.22))
