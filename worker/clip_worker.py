...
def caption_position_override(width: int, height: int, position: str, horizontal: str) -> str:
    """Move centre-row captions below the speaker's face while preserving alignment.

    ASS MarginV does not move middle-aligned text, so middle captions previously
    sat directly over faces. A restrained 58% vertical anchor keeps them lower
    without dropping into TikTok's bottom controls.
    """
    if str(position) != "middle":
        return ""
    x_ratio = {"left": 0.08, "center": 0.50, "right": 0.92}.get(str(horizontal), 0.50)
    return f"{{\\pos({int(round(width * x_ratio))},{int(round(height * 0.58))})}}"
...
def write_ass(candidate: Candidate, template: dict[str, Any], ass_file: Path) -> None:
    ...
    # Keep captions inside a platform-safe horizontal area even when an old
    # template has margins that are too small. Eight percent is 86 px on a
    # 1080-wide vertical clip and prevents outlines/glow from being clipped.
    margin_h = max(int(template.get("captionMarginH", 90)), int(round(width * 0.08)))
    ...
    position_override = caption_position_override(
        width, height, str(template.get("captionPosition", "middle")),
        str(template.get("captionHorizontal", "center")),
    )
...
"""
    ...
    if mode == "dynamic-stack" and words:
        for frame in dynamic_caption_frames(candidate, template):
            ...
            text = "\\N".join(lines)
            events.append(f"Dialogue: 2,{ass_time(frame['start'])},{ass_time(frame['end'])},Caption,,0,0,0,,{position_override}{text}")
        ...
        for group in chunked(words, max_words):
            for active_index, active in enumerate(group):
                ...
                events.append(f"Dialogue: 2,{ass_time(start)},{ass_time(end)},Caption,,0,0,0,,{position_override}{' '.join(text_parts)}")
    else:
        for segment in candidate.segments:
            ...
            events.append(f"Dialogue: 2,{ass_time(start)},{ass_time(end)},Caption,,0,0,0,,{position_override}{text}")
...
def track_speaker_keyframes(
    ...
    * **Speech activity** — `speech_spans` carries the Whisper word timings.
    ...
    def dedupe_faces(items: list[tuple[int, int, int, int]]) -> list[tuple[int, int, int, int]]:
        # Multiple cascades often report the same face several times. Merge close
        # detections so duplicates cannot win the score and make the crop jump.
        kept: list[tuple[int, int, int, int]] = []
        for face in sorted(items, key=lambda f: f[2] * f[3], reverse=True):
            fx, fy, fw, fh = face
            cx, cy = fx + fw / 2.0, fy + fh / 2.0
            if any(abs(cx - (kx + kw / 2.0)) < max(fw, kw) * 0.38 and
                   abs(cy - (ky + kh / 2.0)) < max(fh, kh) * 0.38
    ...
    previous_gray = None
    previous_subject: tuple[float, float] | None = None
    for index in range(samples + 1):
        ...
        if faces:
            ...
            for (fx, fy, fw, fh) in faces:
                ...
                # Prefer continuity unless another face has strong evidence of
                # speaking. This prevents one weak detection from snapping to a
                # listener or cutting the current speaker off.
                face_cx, face_cy = fx + fw / 2.0, fy + fh * 0.42
                if previous_subject is not None:
                    distance = ((face_cx - previous_subject[0]) ** 2 + (face_cy - previous_subject[1]) ** 2) ** 0.5
                    score += max(0.0, 1.0 - distance / max(src_w, src_h)) * 0.32
            ...
            if best:
                fx, fy, fw, fh = best
                # Anchor slightly above the face centre to reserve headroom.
                # Using the geometric centre was more likely to crop hair/forehead.
                subject = (fx + fw / 2.0, fy + fh * 0.42)
                previous_subject = subject
                raw.append((offset, subject[0], subject[1]))
        elif previous_subject is not None:
            # Hold the last reliable speaker through brief detector misses rather
            # than jumping back to the middle of the source.
            raw.append((offset, previous_subject[0], previous_subject[1]))
        previous_gray = gray
    cap.release()
