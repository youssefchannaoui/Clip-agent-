...
def write_ass(candidate: Candidate, template: dict[str, Any], ass_file: Path) -> None:
    ...
    font_size = int(round(float(template.get("captionFontSize", 62)) * max(0.55, min(1.8, float(template.get("captionScale", 1.0))))))
    ...
    custom_caption_position = "captionX" in template or "captionY" in template
    alignment = 5 if custom_caption_position else alignment_for(
        str(template.get("captionPosition", "middle")),
        str(template.get("captionHorizontal", "right")),
    )
    caption_x = int(round(width * max(0.08, min(0.92, float(template.get("captionX", 50)) / 100.0))))
    caption_y = int(round(height * max(0.10, min(0.88, float(template.get("captionY", 62)) / 100.0))))
    caption_position_tag = f"{{\\an5\\pos({caption_x},{caption_y})}}" if custom_caption_position else ""
    caption_offset = max(-0.5, min(0.5, float(template.get("captionTimingOffsetMs", -120)) / 1000.0))
...
"""
    ...
    if mode == "dynamic-stack" and words:
        for frame in dynamic_caption_frames(candidate, template):
            ...
            text = "\\N".join(lines)
            events.append(f"Dialogue: 2,{ass_time(max(0.0, frame['start'] + caption_offset))},{ass_time(max(0.08, frame['end'] + caption_offset))},Caption,,0,0,0,,{caption_position_tag}{text}")
        ...
        for group in chunked(words, max_words):
            for active_index, active in enumerate(group):
                ...
                events.append(f"Dialogue: 2,{ass_time(max(0.0, start + caption_offset))},{ass_time(max(0.08, end + caption_offset))},Caption,,0,0,0,,{caption_position_tag}{' '.join(text_parts)}")
    else:
        for segment in candidate.segments:
            ...
            events.append(f"Dialogue: 2,{ass_time(max(0.0, start + caption_offset))},{ass_time(max(0.08, end + caption_offset))},Caption,,0,0,0,,{caption_position_tag}{text}")
...
def build_video_filter(template: dict[str, Any], ass_file: Path, crop_plan: dict[str, Any] | None = None) -> str:
    ...
    if fit_mode == "crop":
        if crop_plan:
            ...
            zoom = max(0.75, min(2.5, float(template.get("smartFramingZoom", 1.0))))
            pos_x = max(0.0, min(1.0, float(template.get("cropPositionX", 50)) / 100.0))
            pos_y = max(0.0, min(1.0, float(template.get("cropPositionY", 50)) / 100.0))
            scaled_w = max(width, int(round(width * zoom)))
            scaled_h = max(height, int(round(height * zoom)))
            graph = (
                f"[0:v]scale={scaled_w}:{scaled_h}:force_original_aspect_ratio=increase,"
                f"crop={width}:{height}:(iw-ow)*{pos_x:.5f}:(ih-oh)*{pos_y:.5f},setsar=1[base]"
            )
