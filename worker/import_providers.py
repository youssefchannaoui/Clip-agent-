...
def proxy_pool() -> list[str]:
    """Every exit address available to a download, newest source first.
    ...
    per-call read means the fresh pool is live on the next download attempt --
    no container restart, no window where a job must use a dead exit because
    the fix is waiting on a redeploy.
    """
    pool_file = os.getenv("VIDEO_IMPORT_PROXY_FILE", "").strip()
    if pool_file:
        try:
            lines = [line.strip() for line in Path(pool_file).read_text(encoding="utf-8").splitlines() if line.strip()]
            if lines:
                return lines
        except OSError:
            pass
    return [p.strip() for p in os.getenv("VIDEO_IMPORT_PROXIES", "").split(",") if p.strip()]
